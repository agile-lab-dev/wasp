...
/** A named model for data stored as files on a raw datastore (eg HDFS).
  *
  * The `uri` is augmented with time information if `timed` is true. For writers this means whether to use `uri` as-is
  * or create timed namespaces (eg for HDFS, a subdirectory) inside; for readers whether to read from `uri` as-is or
  * from the most recent timed namespace inside.
  *
  * `schema` is a json-encoded DataFrame schema, that is, a StructType. See DataType.fromJson and DataType.json.
  *
  * `options` control the underlying spark DataFrameWriter/Reader in the writers/readers using an instance of this
  * model.
  *
  * @param name
...
/** Options for a raw datastore.
  ...
  *   - "default", like "error" for it.agilelab.bigdata.wasp.consumers.SparkWriter, like "append" for
  *     it.agilelab.bigdata.wasp.consumers.SparkStreamingWriter
  *
