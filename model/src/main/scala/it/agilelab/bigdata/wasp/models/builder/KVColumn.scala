...
object KVColumn {
  ...
  def avro(fieldName: String, qualifier: String, avro: String): AvroKVColumn = AvroKVColumn(fieldName, qualifier, avro)

  // this can be made better to check that obj.fieldName is of type KVType or Option[KVType]
  def primitive(fieldName: String, qualifier: String, primitive: KVType): PrimitiveKVColumn =
    PrimitiveKVColumn(fieldName, qualifier, primitive)

  def primitive(fieldName: String, qualifier: String, primitive: String): PrimitiveKVColumn = {
    KVType
      .get(primitive)
      .map(PrimitiveKVColumn(fieldName, qualifier, _))
...
final case class AvroKVColumn(override val fieldName: String, override val qualifier: String, avro: String)
    extends KVColumn(fieldName, qualifier) {
  override def toJson(cfName: String): String = {
    s""""$fieldName": {"cf": "$cfName", "col": "$qualifier", "avro": "$avro"}"""
  }
}

final case class PrimitiveKVColumn(override val fieldName: String, override val qualifier: String, primitive: KVType)
    extends KVColumn(fieldName, qualifier) {
  override def toJson(cfName: String): String = {
...
object KVColumnMacros {
  def primitiveImpl[T](
      c: blackbox.Context
  )(expr: c.Expr[T], qualifier: c.Expr[String])(implicit tag: c.WeakTypeTag[T]): c.Expr[PrimitiveKVColumn] = {
    import c.universe._
