...
/** The HttpModel used by HttpWriter to send data with HTTP protocol
  ...
  *   The name of the DataFrame column to be used as http headers, it must be of type Map[String,String], if None, no
  *   header will be sent in the request, except for the content-type and content-encoding ones
  * @param valueFieldsNames
  *   The list of DataFrame columns to be rendered as json in the http request body. If the passed list is empty, all
  *   the fields, except the headers field (if any) will be rendered as a json object. If there is only one field, the
  *   behaviour is controlled by the structured field
  * @param compression
  *   The HttpCompression
  * @param mediaType
  ...
  * @param structured
  *   Indicates how the request body will be rendered. The effect of this configuration has effect only if the DataFrame
  *   contains only one column to be sent and only if it is of ArrayType or MapType. If structured is true the array or
  *   map will always be enclosed in a json object, otherwise the map or the array will be at the top level of the json
  *   document. Input dataframe:
  *   {{{
  *                   +---------+
  *                   |  values |
  *                   +---------+
