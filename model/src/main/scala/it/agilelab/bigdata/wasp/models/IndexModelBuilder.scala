...
/** A builder able to create instances of [[IndexModel]].
  ...
  * @param config
  *   The current index config
  * @param isRolling
  *   Whether the index should be rolling
  * @param id
  *   Wether the index has an id field
  * @param options
  *   Wether the index has an options field
  * @tparam Stage
...
class IndexModelBuilder[Stage <: IndexModelBuilder.Stage, Kind <: IndexModelBuilder.DataStoreKind] private (
  ...
  /** Builds an index model, this method is callable only when Stage is a subclass of
    * [[IndexModelBuilder.Stage.Complete]] (has more trait mixed but not less).
    *
    * @param evidence
...
object IndexModelBuilder {
  ...
  object Elastic {
    ...
    /** An elastic config.
      ...
      * @param pushdownQuery
      *   The query to use in spark query pushdown
      */
  ...
  object Solr {
    ...
    /** Class representing a Field of Solr Index.
      ...
      * @param omitNorms
      *   If true, omits the norms associated with this field (this disables length normalization for the field, and
      *   saves some memory). Defaults to true for all primitive (non-analyzed) field types, such as int, float, data,
      ...
      *   boost for fields that don’t require that information. It also reduces the storage space required for the
      *   index. Queries that rely on position that are issued on a field with this option will silently fail to find
      *   documents. This property defaults to true for all field types that are not text fields.
      * @param omitPositions
      ...
      * @param required
      *   Instructs Solr to reject any attempts to add a document which does not have a value for this field. This
      *   property defaults to false.
      * @param useDocValuesAsStored
      *   If the field has docValues enabled, setting this to true would allow the field to be returned as if it were a
      *   stored field (even if it has stored=false) when matching “*” in an fl parameter.
      * @param large
      *   Large fields are always lazy loaded and will only take up space in the document cache if the actual value is <
      *   512KB. This option requires stored="true" and multiValued="false". It’s intended for fields that might have
      *   very large values so that they don’t get cached in memory.
      * @tparam A
    ...
    object Type {
      ...
      /** Spatial Search: a latitude/longitude coordinate pair; possibly multi-valued for multiple points. Usually it’s
      ...
      /** String (UTF-8 encoded string or Unicode). Strings are intended for small fields and are not tokenized or
        * analyzed in any way. They have a hard limit of slightly less than 32K.
        */
      ...
      /** Date field. Represents a point in time with millisecond precision. See the section Working with Dates.
        * precisionStep="0" minimizes index size; precisionStep="8" (the default) enables more efficient range queries.
        * For single valued fields, use docValues="true" for efficient sorting.
        */
      case object TrieDate extends Type[Instant]("pdate")

      /** Double field (64-bit IEEE floating point). precisionStep="0" minimizes index size; precisionStep="8" (the
        * default) enables more efficient range queries. For single valued fields, use docValues="true" for efficient
        * sorting.
        */
      ...
      /** Integer field (32-bit signed integer). precisionStep="0" enables efficient numeric sorting and minimizes index
        * size; precisionStep="8" (the default) enables efficient range queries. For single valued fields, use
        * docValues="true" for efficient sorting.
        */
