...
final case class PrefixAndTimeBoundKeyValueMatchingStrategy(separator: String, pattern: String, locale: String = "UTC")
...
final case class TimeBasedBetweenPartitionPruningStrategy(
    columnName: String,
  ...
  private def getDatesToConsider(
      startRefDate: ZonedDateTime,
      endRefDate: ZonedDateTime,
      dateFormatter: DateTimeFormatter,
      granularity: TemporalUnit,
      dateToConsider: List[String] = Nil
  ): List[String] = {
...
trait DataStoreConfJsonSupport extends DefaultJsonProtocol {
  ...
  def createDataStoreConfFormat: RootJsonFormat[DataStoreConf] = {
    ...
    implicit val exactKeyValueMatchingStrategyFormat: RootJsonFormat[ExactKeyValueMatchingStrategy] = jsonFormat0(
      ExactKeyValueMatchingStrategy.apply
    )
    implicit val prefixKeyValueMatchingStrategyFormat: RootJsonFormat[PrefixKeyValueMatchingStrategy] = jsonFormat0(
      PrefixKeyValueMatchingStrategy.apply
    )
    implicit val prefixAndTimeBoundKeyValueMatchingStrategyFormat
        : RootJsonFormat[PrefixAndTimeBoundKeyValueMatchingStrategy] = jsonFormat3(
      PrefixAndTimeBoundKeyValueMatchingStrategy.apply
    )
    implicit val keyValueMatchingStrategyFormat: RootJsonFormat[KeyValueMatchingStrategy] =
      new RootJsonFormat[KeyValueMatchingStrategy] {
        override def read(json: JsValue): KeyValueMatchingStrategy = {
          json.asJsObject.getFields("type") match {
            ...
            case Seq(JsString("PrefixAndTimeBoundKeyValueMatchingStrategy")) =>
              json.convertTo[PrefixAndTimeBoundKeyValueMatchingStrategy]
    ...
    implicit val timeBasedBetweenPartitionPruningStrategyFormat
        : RootJsonFormat[TimeBasedBetweenPartitionPruningStrategy] =
      jsonFormat(
    ...
    implicit val partitionPruningFormat: RootJsonFormat[PartitionPruningStrategy] =
      new RootJsonFormat[PartitionPruningStrategy] {
        override def read(json: JsValue): PartitionPruningStrategy = json.asJsObject.getFields("type") match {
          case Seq(JsString("TimeBasedBetweenPartitionPruningStrategy")) =>
            json.convertTo[TimeBasedBetweenPartitionPruningStrategy]
    ...
    implicit val exactRawMatchingStrategyFormat: RootJsonFormat[ExactRawMatchingStrategy] = jsonFormat1(
      ExactRawMatchingStrategy.apply
    )
    implicit val prefixRawMatchingStrategyFormat: RootJsonFormat[PrefixRawMatchingStrategy] = jsonFormat1(
      PrefixRawMatchingStrategy.apply
    )
    implicit val containsRawMatchingStrategyFormat: RootJsonFormat[ContainsRawMatchingStrategy] = jsonFormat1(
      ContainsRawMatchingStrategy.apply
    )
    implicit val rawMatchingStrategyFormat: RootJsonFormat[RawMatchingStrategy] =
      new RootJsonFormat[RawMatchingStrategy] {
