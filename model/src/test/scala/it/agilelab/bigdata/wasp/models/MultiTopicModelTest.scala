...
class MultiTopicModelTest extends FunSuite {
  ...
  test("Test that topic model compression validation works") {
    ...
    val topicsWithSameCompression =
      Vector.range(0, 10).map(x => (s"topic$x", TopicCompression.Gzip)).map(topicForNameAndCompression.tupled)
    ...
    val topicsWithDifferentCompression = Vector
      .range(0, 4)
      .map {
    ...
    val differentCompressionResult =
      MultiTopicModel.validateTopicModelsHaveSameCompression(topicsWithDifferentCompression)
    ...
    assert(
      message === "All topic models must have the same compression setting, found settings: [topic0,topic2] use snappy,[topic1,topic3] use gzip"
    )
