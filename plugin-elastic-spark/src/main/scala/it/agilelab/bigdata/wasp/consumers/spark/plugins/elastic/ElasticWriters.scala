...
class ElasticSparkStreamingWriter(indexBL: IndexBL,
  ...
  extends SparkStreamingWriter with ElasticConfiguration with Logging {
  ...
  override def write(stream: DStream[String]): Unit = {
    ...
    if (indexOpt.isDefined) {
      ...
      if (??[Boolean](elasticAdminActor, CheckOrCreateIndex(indexName, index.name, index.dataType, index.getJsonSchema))) {
        val resourceBroadcast = ssc.sparkContext.broadcast(index.resource)
        val address = elasticConfig.connections.filter(_.metadata.flatMap(_.get("connectiontype")).getOrElse("") == "rest").mkString(",")

        val optionsBroadcasted = ssc.sparkContext.broadcast(Map("es.nodes" -> address, "es.input.json" -> "true", "es.batch.size.entries" -> "1"))

        logger.info(s"Write to elastic with spark streaming. Configuration passed: options: ${optionsBroadcasted.value}, resource: ${resourceBroadcast.value}")
...
class ElasticSparkWriter(indexBL: IndexBL,
                         ...
                         id: String,
                         elasticAdminActor: ActorRef)
  extends SparkWriter with ElasticConfiguration with Logging {
  ...
  override def write(data: DataFrame): Unit = {
    ...
    if (indexOpt.isDefined) {
      ...
      if (??[Boolean](elasticAdminActor, CheckOrCreateIndex(indexName, index.name, index.dataType, index.getJsonSchema))) {
        ...
        val options = Map("es.nodes" -> addressBroadcast.value, /* "es.input.json" -> "true",*/ "es.batch.size.entries" -> "1")
