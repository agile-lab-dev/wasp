...
class PackageTest extends FlatSpec with Matchers with SparkSuite {
  ...
  it should "test getAllDataFrames" in {
    ...
    dataFrames
      ...
      .select("someNumber")
      .take(10)
      .map(_.getInt(0)) should contain theSameElementsAs (1 to 10)
    dataFrames
      .reduce(_.union(_))
      .where("name == 'test_2'")
      .select("someNumber")
  ...
  it should "test getFirstDataFrame" in {
    val map       = (1 to 2).map(i => ReaderKey(s"key_$i", "2") -> createDataframe(spark, s"test_$i", 1 to 10)).toMap
  ...
  it should "test getAllReaderKeys" in {
    val map        = (1 to 2).map(i => ReaderKey(s"key_$i", "2") -> createDataframe(spark, s"test_$i", 1 to 10)).toMap
  ...
  it should "test getFirstReaderKey" in {
    val map        = (1 to 2).map(i => ReaderKey(s"key_$i", "2") -> createDataframe(spark, s"test_$i", 1 to 10)).toMap
  ...
  it should "test filterBySourceTypeName" in {
  ...
  it should "test filterByName" in {
    val map        = (1 to 2).map(i => ReaderKey(s"key_$i", s"$i") -> createDataframe(spark, s"test_$i", 1 to 10)).toMap
  ...
  it should "test filterByName with a wrong name" in {
    val map        = (1 to 2).map(i => ReaderKey(s"key_$i", s"$i") -> createDataframe(spark, s"test_$i", 1 to 10)).toMap
  ...
  it should "test getByReaderKey wrong" in {
    val map = (1 to 2).map(i => ReaderKey(s"key_$i", s"$i") -> createDataframe(spark, s"test_$i", 1 to 10)).toMap
