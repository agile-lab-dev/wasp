...
class ActivationStepsTest extends FlatSpec with Matchers with SparkSetup {
  ...
  "this" should "not create a strategy" in withSparkSession { ss=>
    ...
    val etl = StructuredStreamingETLModel("name","dafault",null,List.empty,
      null,List.empty,None,None)
  ...
  "this" should "throw a exception" in withSparkSession { ss=>
    ...
    val etl = StructuredStreamingETLModel("name","dafault",null,List.empty,
      null,List.empty,Some(strategyModel),None)
    val strategyTry = aSM.createStrategy(etl)
  ...
  "this" should "create a FreeCodeStrategy with config wrong" in withSparkSession { ss=>
    val aSM = new ActivationStepsMock(ss)
    val strategyModel = StrategyModel("it.agilelab.bigdata.wasp.consumers.spark.strategies.FreeCodeStrategy",Some(s"""{XXX:"test"}"""))
    val etl = StructuredStreamingETLModel("name","dafault",null,List.empty,
      null,List.empty,Some(strategyModel),None)
    val strategyTry = aSM.createStrategy(etl)
    strategyTry.isFailure shouldBe true
  ...
  "this" should "create a FreeCodeStrategy without FreeCodeBL" in withSparkSession { ss=>
    ...
    val strategyModel = StrategyModel("it.agilelab.bigdata.wasp.consumers.spark.strategies.FreeCodeStrategy",Some(s"""{name:"test"}"""))
    val etl = StructuredStreamingETLModel("name","dafault",null,List.empty,
      null,List.empty,Some(strategyModel),None)
    val strategyTry = aSM.createStrategy(etl)
    strategyTry.isFailure shouldBe true
  ...
  "this" should "create a FreeCodeStrategy with freeCode wrong" in withSparkSession { ss=>
    ...
    val strategyModel = StrategyModel("it.agilelab.bigdata.wasp.consumers.spark.strategies.FreeCodeStrategy",Some(s"""{name:"${freeCodeModel.name}"}"""))
  ...
  "this" should "create a correct FreeCodeStrategy with columns wrong" in withSparkSession { ss=>
    val aSM = new ActivationStepsMock(ss)
    val freeCodeModel = FreeCodeModel("test","""dataFrames.getFirstDataFrame.withColumn("column_test",concat(col("column_test"),lit("pippo"))) """)
    val strategyModel = StrategyModel("it.agilelab.bigdata.wasp.consumers.spark.strategies.FreeCodeStrategy",Some(s"""{name:"${freeCodeModel.name}"}"""))
  ...
  "this" should "create a correct FreeCodeStrategy" in withSparkSession { ss=>
    val aSM = new ActivationStepsMock(ss)
    val freeCodeModel = FreeCodeModel("test","""dataFrames.getFirstDataFrame.withColumn("column_test",concat(col("_2"),lit("pippo"))) """)
    val strategyModel = StrategyModel("it.agilelab.bigdata.wasp.consumers.spark.strategies.FreeCodeStrategy",Some(s"""{name:"${freeCodeModel.name}"}"""))
    ...
    output.rdd.collect().toSeq.map(r =>r.getAs[String]("column_test")) should contain theSameElementsAs List("1pippo","2pippo","3pippo")
  }

}

class ActivationStepsMock(override protected val sparkSession: SparkSession,override val freeCodeBL: FreeCodeBL = new FreeCodeBLMock) extends ActivationSteps {
