...
class NodeLabelsSchedulingStrategySpec
  ...
  "Node label scheduling strategy" must {
    ...
    "Correctly maintain downstream tie breaker strategies " in {
      val initialSchedulingStrategy: SchedulingStrategy =
        NodeLabelsSchedulingStrategy(Map.empty, new FifoSchedulingStrategyFactory)

      val unschedulableCollaborators: Set[Collaborator] = Seq
        .range(0L, 3L)
        .map { i =>
      ...
      val schedulableCollaborators = Seq
        .range(0L, 3L)
        .map { i =>
      ...
      val beforeLosingTwoOfTheThreeSchedulableNodesSchedulingStrategy =
        Seq.range(0L, 100L).map(_ % schedulableCollaborators.size).foldLeft(initialSchedulingStrategy) {
          case (currentSchedulingStrategy, currentCollaboratorIndex) =>
            val (chosenCollaborator, updatedSchedulingStrategy) =
              currentSchedulingStrategy.choose(allCollaborators, pipegraph).right.value
            chosenCollaborator.address.longUid should be(
              allCollaborators.toSeq.apply(currentCollaboratorIndex.toInt).address.longUid
            )
            updatedSchedulingStrategy
        }

      val theOnlyStillSchedulableCollaborator =
        Collaborator(UniqueAddress(Address("tcp", "wasp"), 199L), null, Set("consumer", "label-1", "label-2"))
      val withOnlyOneStillSchedulable: Set[Collaborator] =
        unschedulableCollaborators + theOnlyStillSchedulableCollaborator

      val beforeLosingTheLastSchedulableNode =
        Seq.range(0L, 100L).map(_ => 0).foldLeft(beforeLosingTwoOfTheThreeSchedulableNodesSchedulingStrategy) {
          case (currentSchedulingStrategy, currentCollaboratorIndex) =>
            val (chosenCollaborator, updatedSchedulingStrategy) =
              currentSchedulingStrategy.choose(withOnlyOneStillSchedulable, pipegraph).right.value
      ...
      Seq.range(0L, 100L).map(_ => 0).foldLeft(beforeLosingTheLastSchedulableNode) {
        case (currentSchedulingStrategy, currentCollaboratorIndex) =>
          val (chosenCollaborator, updatedSchedulingStrategy) =
            currentSchedulingStrategy.choose(noSchedulableNodes, pipegraph).left.value
    ...
    "break ties using Lowest Node UUID strategy" must {
      ...
      "choose always the node with the lowest UUID" in {
        ...
        val initialSchedulingStrategy: SchedulingStrategy =
          NodeLabelsSchedulingStrategy(Map.empty, new LowestUUIDNodeSchedulingStrategyFactory)
