...
import it.agilelab.bigdata.wasp.consumers.spark.strategies.gdpr.config.{
  DeletionConfig,
  HBaseDeletionConfig,
  HdfsDeletionConfig
}
...
class GdprStrategy(dataStores: List[DataStoreConf]) extends Strategy with HasPostMaterializationHook with Logging {
  ...
  /** @param maybeDataframe
    ...
    * @return
    *   the wanted outcome of the strategy. This means that if the maybeError is not None and the hook does not have a
    *   "recovery" logic, it should always return a Failure wrapping the input error.
    */
  ...
  override def transform(dataFrames: Map[ReaderKey, DataFrame]): DataFrame = {
    ...
    val storageLevel =
      ConfigUtils
        .getOptionalString(configuration, STORAGE_LEVEL)
        .flatMap { s =>
          Try(StorageLevel.fromString(s)).recoverWith { case e =>
    ...
    val deletionResults: RDD[DeletionOutput] = deletionConfigs
      .map {
  ...
  private def hdfsFailureOutput(hdfsConfig: HdfsDeletionConfig, exception: Throwable): Seq[DeletionOutput] = {
    ...
    hdfsConfig.keysToDeleteWithCorrelation.map { keyWithCorrelation =>
      DeletionOutput(
        keyWithCorrelation,
        keyMatchType,
        HdfsRawModelSource(hdfsConfig.rawModel.uri),
  ...
  private def hBaseFailureOutput(hbaseConfig: HBaseDeletionConfig, exception: Throwable): Seq[DeletionOutput] = {
    ...
    hbaseConfig.keysWithScan.collect().map { case (keyWithCorrelation, _) =>
      DeletionOutput(
        keyWithCorrelation,
        keyMatchType,
        HBaseTableSource(hbaseConfig.tableName),
        DeletionFailure(exception)
      )
    }
  }
