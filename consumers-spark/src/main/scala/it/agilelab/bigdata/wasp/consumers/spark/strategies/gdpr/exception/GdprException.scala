...
case class FailureDuringBackupRestorationException(
    deletionException: DeletionException,
    restorationThrowable: Throwable
) extends Exception(restorationThrowable.getLocalizedMessage, restorationThrowable.initCause(deletionException))
    with GdprException

case class FailureDuringBackupDeletionAfterRestorationException(
    deletionException: DeletionException,
    backupDeletionThrowable: Throwable
