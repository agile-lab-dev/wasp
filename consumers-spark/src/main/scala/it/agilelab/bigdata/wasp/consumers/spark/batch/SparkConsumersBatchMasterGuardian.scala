...
object SparkConsumersBatchMasterGuardian {
  ...
  /** Creates exitingWatchdogProps for the [SparkConsumersBatchMasterGuardian] actor
    ...
    * @param env
    *   The environment
    * @param sparkWriterFactory
    *   The spark writer factory
    * @param plugins
    *   The plugins
    * @return
    *   the exitingWatchdogProps of the [[SparkConsumersBatchMasterGuardian]] actor
    */
  def props(
      env: {
        val batchJobBL: BatchJobBL; val batchSchedulersBL: BatchSchedulersBL; val indexBL: IndexBL; val rawBL: RawBL;
    ...
    props(
      env.batchJobBL,
      env.batchSchedulersBL,
      10,
      refFactory => refFactory.actorOf(BatchJobActor.props(env, sparkWriterFactory, plugins, retryDuration))
    )
  }
  ...
  /** Creates exitingWatchdogProps for the actor, use this method in tests to plug into the instantiation of children
    * actor via the [[BatchJobActorFactory]] parameter
    * @param batchJobBl
    *   The bl to access batch jobs
    * @param batchSchedulersBL
    *   The bl to access batch schedulers
    * @param howManySlaveActors
    *   How many slave actor to start
    * @param batchJobActorFactory
    *   The factory used to instantiate actors
    * @return
    *   the exitingWatchdogProps of the [[SparkConsumersBatchMasterGuardian]] actor
    */
  private[batch] def props(
      batchJobBl: BatchJobBL,
      batchSchedulersBL: BatchSchedulersBL,
      howManySlaveActors: Int,
      batchJobActorFactory: BatchJobActorFactory
  ): Props =
    Props(
      new SparkConsumersBatchMasterGuardian(batchJobBl, batchSchedulersBL, howManySlaveActors, batchJobActorFactory)
    )
  ...
  /** Sent from [[SparkConsumersBatchMasterGuardian]] to assign a Job to the requesting [[BatchJobActor]]
    *
    * @param model
    *   The model of the batch job
    * @param instance
    *   The model of the instance
    */
  private[batch] case class Job(model: BatchJobModel, instance: BatchJobInstanceModel)

  /** Sent from [[SparkConsumersBatchMasterGuardian]] to inform the requesting [[BatchJobActor]] that an error occurred
    * while orchestrating the creation and assignment of an instance.
    *
    * @param error
  ...
  /** Sent from [[BatchJobActor]] to [[SparkConsumersBatchMasterGuardian]] to inform it that job succeeded
    * @param model
    *   THe model of the job that succeeded
    * @param instance
    *   The instance of the job that succeeded
    */
  ...
  /** Sent from [[BatchJobActor]] to [[SparkConsumersBatchMasterGuardian]] to inform it that a job failed
    * @param model
    *   THe model of the job that failed
    * @param instance
    *   The instance of the job that failed
    */
...
private[batch] trait DatabaseOperations {
  ...
  /** Retrieves [[BatchJobModel]]s from the db.
    *
    * @return
    *   A Try containing the [[BatchJobModel]]s or an exception
    */
  ...
  /** Retrieves [[BatchJobInstanceModel]]s with specified statuses
    ...
    * @return
    *   A Try containing the [[BatchJobInstanceModel]]s or an exception
    */
  ...
  /** Updates the status of the supplied instances to the target status.
    ...
    */
  def updateToStatus(
      jobInstances: Seq[BatchJobInstanceModel],
      targetStatus: JobStatus
  ): Try[Seq[BatchJobInstanceModel]] = {
  ...
  /** Updates the status of the supplied instance to the target status.
    ...
    */
  def updateToStatus(
      jobInstance: BatchJobInstanceModel,
      targetStatus: JobStatus,
      maybeError: Option[Throwable] = None
  ) = Try {

    val updated = maybeError
      .map { error => ExceptionUtils.getStackTrace(error) }
...
class SparkConsumersBatchMasterGuardian private (
      ...
      schedules foreach { schedule =>
        {
          scheduler.scheduleJob(
            schedule.getQuartzJob(sparkConsumersBatchMasterGuardianActorPath),
            schedule.getQuartzTrigger
          )
        }
      }
    }
  ...
  /** Behavior of this actor before initialization completes
    *
    * @param retryTimes
    *   How many time should retry initialization
    * @param retryDuration
    *   The delay between attempts
    * @return
    *   The behavior of the actor
    */
  private def uninitialized(retryTimes: Int, retryDuration: FiniteDuration): Receive = {
    ...
    case Initialize => {
      ...
      initializationResult match {
        ...
        case Success((pending, schedules)) => {
          ...
          val watchedChildren = List
            .fill(howManySlaveActors) {
  ...
  /** The main behavior of the actor, composes internal and external behavior.
    *
    * @param pendingJobs
    *   The jobs that are currently pending
    * @param runningJobs
    *   THe jobs that are currently running associated with the actor that is executing them
    * @param children
    *   The children actor that are executing jobs
    * @return
    *   The behavior
    */
  private def behavior(
      pendingJobs: Set[BatchJobInstanceModel],
      runningJobs: Map[BatchJobInstanceModel, ActorRef],
      children: Set[ActorRef]
  ): Receive = PartialFunction.empty
  ...
  private def externalBehavior(
    ...
    case BatchMessages.StartBatchJob(name, restConfig) => {
        ...
        bjModel.forall { bjm =>
          ...
          (
            (!bjm.exclusivityConfig.isFullyExclusive) &&
              ...
              jobs.exists { j =>
                j.instanceOf == name &&
                bjm.exclusivityConfig.restConfigExclusiveParams.forall { exclusiveParam =>
      ...
      if (bjmRetrieved && isInConflict) {
        ...
        sender() ! BatchMessages.StartBatchJobResultFailure(
          name,
        ...
        batchJobModel.flatMap(createInstanceOf(_, restConfig)) match {
          ...
          case Failure(error) => {
            sender() ! BatchMessages.StartBatchJobResultFailure(
              name,
              s"failure creating new batch job instance [${error.getMessage}]"
            )
          }
        }
      }
    ...
    case BatchMessages.StopBatchJob(name) => {
      ...
      if (!pendingJobs.exists(_.instanceOf == name) && !runningJobs.keys.exists(_.instanceOf == name)) {
        sender() ! BatchMessages.StopBatchJobResultFailure(
          name,
          s"Cannot stop job [$name] whose instances are not running or pending"
        )
      } else {
        retrieveBatchJob(name)
          ...
          case Success((original: Seq[BatchJobInstanceModel], _)) => {
            ...
            original.foreach {
              case instance if instance.status == JobStatus.PENDING =>
                context.become(behavior(pendingJobs - instance, runningJobs, children))
              case instance if instance.status == JobStatus.PROCESSING =>
          ...
          case Failure(error) => {
            sender() ! BatchMessages.StopBatchJobResultFailure(
              name,
              s"failure stopping instances of job [${error.getMessage}]"
            )
          }
        }
      }
  ...
  private def internalBehavior(
    ...
    case GimmeOneJob if pendingJobs.nonEmpty => {
      ...
      retrieveBatchJob(original.instanceOf).flatMap { model =>
        retrieveBatchJobInstances(model)(JobStatus.PENDING).flatMap { instances =>
          updateToStatus(instances.head, JobStatus.PROCESSING).map { instance =>
            (model, instance)
          }
        }
    ...
    case JobSucceeded(model, instance) => {
      ...
      updateToStatus(instance, JobStatus.SUCCESSFUL) match {
        case Failure(error) => {
          ...
          context.system.scheduler.scheduleOnce(retryDuration, self, JobSucceeded(model, instance))(
            context.dispatcher,
            originalSender
          )
        }
    ...
    case JobFailed(model, instance, error) => {
      ...
      updateToStatus(instance, JobStatus.FAILED, Some(error)) match {
        case Failure(e) => {
          ...
          context.system.scheduler.scheduleOnce(retryDuration, self, JobFailed(model, instance, e))(
            context.dispatcher,
            originalSender
          )
        }
