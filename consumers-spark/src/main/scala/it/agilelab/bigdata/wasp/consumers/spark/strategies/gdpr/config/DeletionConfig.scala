...
import it.agilelab.bigdata.wasp.consumers.spark.strategies.gdpr.exception.ConfigExceptions.{
  KeyValueConfigException,
  RawDataConfigException
}
...
/** Contains the configuration settings for an HBase Deletion Job
  *
  * @param keysWithScan
  *   list of distinct keys to delete (from config or input model), together with the HBase scan that will be used to
  *   match the HBase rowKeys to delete
  * @param tableName
  *   namespace:table name of the HBase table to handle derived from the KeyValueModel
  * @param hbaseConfigModel
  *   HBaseConfigModel that will be used to create an HBaseConnection inside the executors
  * @param keyValueMatchingStrategy
  ...
  private def rawMatchingCondition(keysToDelete: Seq[String], rawMatchingStrategy: RawMatchingStrategy): Column = {
    rawMatchingStrategy match {
      ...
      case ContainsRawMatchingStrategy(dataframeKeyMatchingExpression) =>
        ...
        keysToDelete.foldLeft(lit(false)) { case (acc, key) =>
          acc or expression.contains(key)
        }
    }
  }

  /* Generate the partition pruning condition based on the PartitionPruningStrategy defined and the configuration */
  private def partitionPruningCondition(
...
object HBaseDeletionConfig extends Logging {
  ...
  def create(
    ...
    val rowKeys =
      ConfigUtils.keysToDeleteRDD(inputKeys, maybeConfig, KEYS_TO_DELETE_KEY, CORRELATION_ID_KEY).distinct.map {
    ...
    val keysWithScan: RDD[(RowKeyWithCorrelation, Scan)] = keyValueDataStoreConf.keyValueMatchingStrategy match {
      ...
      case prefixAndTime: PrefixAndTimeBoundKeyValueMatchingStrategy =>
        val config = maybeConfig match {
          case Some(x) => x
          case None =>
            throw KeyValueConfigException(
              new ConfigException.Missing(KV_CONF_KEY),
    ...
    val tableName = KeyValueModel.extractTableName(keyValueDataStoreConf.keyValueModel.tableCatalog) match {
      case Failure(exception) =>
        throw KeyValueConfigException(exception, "Impossible to extract HBase table name from KeyValueModel")
  ...
  private def scanExact(
      keysToDelete: RDD[RowKeyWithCorrelation],
      conf: Option[Config]
  ): RDD[(RowKeyWithCorrelation, Scan)] = {
  ...
  private def scanPrefixWithTime(
      config: Config,
      keysToDelete: RDD[RowKeyWithCorrelation],
      matchingStrategy: PrefixAndTimeBoundKeyValueMatchingStrategy
  ): RDD[(RowKeyWithCorrelation, Scan)] = {
    val formatter: DateTimeFormatter =
      DateTimeFormatter.ofPattern(matchingStrategy.pattern, Locale.forLanguageTag(matchingStrategy.locale))
    ...
    keysToDelete.map { case rowKeyWithCorrelation @ RowKeyWithCorrelation(rowKey, _) =>
      ...
      logger.info(
        s"Scan start: '${new String(startRow, StandardCharsets.UTF_8)}', scan stop: '${new String(stopRow, StandardCharsets.UTF_8)}'"
      )
