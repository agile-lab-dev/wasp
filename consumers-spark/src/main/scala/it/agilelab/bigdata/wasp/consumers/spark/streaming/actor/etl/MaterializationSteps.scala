...
trait MaterializationSteps {
  ...
  protected def materialize(
    ...
    for {
      config <- retrieveSparkStreamingConfig.recoverWith { case e: Throwable =>
                  Failure(new Exception(s"Cannot retrieve Spark Streaming config for etl ${etl.name}", e))
                }
      checkpointDir <- generateCheckPointDir(etl, config, pipegraph).recoverWith { case e: Throwable =>
                         Failure(new Exception(s"Cannot generate checkpoint dir name for etl ${etl.name}", e))
                       }
      sparkStructuredStreamingWriter <-
        createWriter(etl).recoverWith { case e: Throwable =>
          Failure(new Exception(s"Cannot create Spark Structured Streaming writer for etl ${etl.name}", e))
        }
      dataStreamWriter <- write(sparkStructuredStreamingWriter, dataFrame).recoverWith { case e: Throwable =>
                            Failure(new Exception(s"Cannot create DataStreamWriter for etl ${etl.name}", e))
                          }
      streamingQuery <-
        startQuery(dataStreamWriter, config, etl, queryName(etl, pipegraph), checkpointDir).recoverWith {
          case e: Throwable => Failure(new Exception(s"Cannot materialize etl ${etl.name}", e))
        }
    } yield streamingQuery

  private def generateCheckPointDir(
      etl: StructuredStreamingETLModel,
  ...
  private def queryName(etl: StructuredStreamingETLModel, pipegraph: PipegraphModel) =
    generateUniqueComponentName(pipegraph, etl)
...
object MaterializationSteps {
  ...
  /** A function able to go from a [[WriterModel]] to an [[Option]] of [[SparkStructuredStreamingWriter]].
    ...
    */
  type WriterFactory =
    (StructuredStreamingETLModel, WriterModel, SparkSession) => Option[SparkStructuredStreamingWriter]
}
