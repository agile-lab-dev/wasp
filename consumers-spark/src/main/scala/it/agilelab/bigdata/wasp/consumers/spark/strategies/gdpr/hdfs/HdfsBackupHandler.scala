...
class HdfsBackupHandler(fs: FileSystem, backupParentDirPath: Path, dataPath: Path) {
  ...
  /** Restores files backup inside `backupPath` to `dataPath`. Each of the files listed recursively inside `backupPath`
    * is "moved back" to the data directory, by replacing the prefix of the backup directory with the prefix of the data
    * directory. Example: `backupPath` = "/user/backup_123" `backupParentDir` = "/user" `dataPath` = "/user/data"
    *
    *   - Listed backup file "/user/backup_123/p1=a/p2=b/file.parquet"
  ...
  private def renameAndAccumulate(backupPath: Path)(acc: Boolean, file: LocatedFileStatus): Boolean = {
    if (!file.isDirectory) {
