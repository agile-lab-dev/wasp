...
class HdfsDataDeletion(fs: FileSystem) extends Logging {
  ...
  def delete(
      deletionHandler: HdfsDeletionHandler,
      backupHandler: HdfsBackupHandler,
      config: HdfsDeletionConfig,
      dataPath: Path,
      spark: SparkSession
  ): Try[Seq[DeletionOutput]] = {
    for {
      filesWithKeys <- if (config.missingPathFailure || fs.exists(new Path(config.rawModel.uri))) {
                         ...
                         Success(
                           config.keysToDeleteWithCorrelation
  ...
  private def deleteOrRollback(
      hdfsDeletionHandler: HdfsDeletionHandler,
    ...
    hdfsDeletionHandler.delete(filesToHandle) recoverWith { case deletionThrowable =>
      ...
      backupHandler.restoreBackup(backupDir) match {
        case Failure(restorationThrowable) =>
          logger.error(
            s"ILLEGAL STATE: exception during backup restoration of $backupDir to $dataPath after deletion failure."
          )
  ...
  private def deleteBackup(backupHandler: HdfsBackupHandler, backupPath: Path): Try[Unit] = {
    ...
    backupHandler.deleteBackup(backupPath).recoverWith { case t =>
  ...
  private def getFilesToFilter(
      config: HdfsDeletionConfig,
      spark: SparkSession
  ): Try[Array[(KeyWithCorrelation, Option[FileName])]] = {
    ...
    val tryFiles = for {
      ...
      _ = logger.info(
            s"Files to filter: ${filesToFilterAndKeys.collect { case (_, Some(x)) => x }.mkString("\n", "\n", "")}"
          )
    } yield filesToFilterAndKeys

    tryFiles.recoverWith { case t =>
      Failure(DeletionException(t))
    }
  }
  ...
  private def filterDataFrame(config: HdfsDeletionConfig, rawDataDF: DataFrame)(implicit
      ev: Encoder[(String, String, String)]
  ): Try[Array[(KeyWithCorrelation, Option[FileName])]] = {
    Try {
      ...
      val outDf = inputFileAndMatchingColumn
        .join(
  ...
  private def makeTheJointOuter(
      config: HdfsDeletionConfig,
      collectedMatches: Array[(FileName, String, String)]
  ): Array[(KeyWithCorrelation, Option[FileName])] = {
    ...
    collectedMatches
      .foldLeft(allInputAsMap) { case (acc, (file, key, corrId)) =>
  ...
  private def createOutput(
      config: HdfsDeletionConfig,
      filesFilteredAndKeys: Array[(KeyWithCorrelation, Option[FileName])]
  ): Seq[DeletionOutput] = {
    val matchType: HdfsMatchType = HdfsMatchType.fromRawMatchingStrategy(config.rawMatchingStrategy)

    filesFilteredAndKeys
      .groupBy(_._1)
      .map { case (keyWithCorrelation, fileNames) =>
        fileNames match {
          case Array((_, None)) =>
            // not found
