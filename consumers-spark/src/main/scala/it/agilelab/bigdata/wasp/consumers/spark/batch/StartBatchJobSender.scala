...
/** A simple Quartz 2 job that sends a StartBatchJobMessage to the configured SparkConsumersBatchMasterGuardian with the
  * configured job id.
  *
  * @author
  *   Nicolò Bidotti
...
class StartBatchJobSender() extends Job {
  ...
  @BeanProperty
  var sparkConsumersBatchMasterGuardianActorPath: String =
    _ // automatically populated by quartz with the corresponding JobDataMap key-value
