...
object BatchJobEtlExecution {
  ...
  def stepCreateStrategy(etl: BatchETL, restConfig: Config, sparkContext: SparkContext): Try[Option[Strategy]] =
    etl match {
  ...
  def stepCreateMlModelsBroadcast(
      mlModelsDB: MlModelsDB,
      batchETL: BatchETL,
      sparkContext: SparkContext
  ): Try[MlModelsBroadcastDB] = batchETL match {
    case batchETLModel: BatchETLModel =>
      Try {
  ...
  private def createStrategy(
      batchETLModel: BatchETLModel,
      restConfig: Config,
      sparkContext: SparkContext
  ): Try[Option[Strategy]] = batchETLModel.strategy match {
  ...
  private def createGdprStrategy(
      batchETLModel: BatchGdprETLModel,
      restConfig: Config,
      sparkContext: SparkContext
  ): Try[Some[GdprStrategy]] = {
    val result = Try {
      Class
        .forName(batchETLModel.strategy.className)
        .getConstructor(classOf[List[DataStoreConf]])
        .newInstance(batchETLModel.strategy.dataStoresConf)
        .asInstanceOf[GdprStrategy]
    }
    val configuration =
      restConfig.withFallback(batchETLModel.strategy.configurationConfig().getOrElse(ConfigFactory.empty()))
    result.map(_.configuration = configuration)
    result.map(_.sparkContext = Some(sparkContext))
    result.map(Some(_))
  }
