...
class MlModelsDB(env: { val mlModelBL: MlModelBL }) {
  ...
  /** This method can be use to retrieve a complete model with own info, if there isn't a timestamp it will take the
    * most recent
    * @param name
    * @param version
    * @param timestampOpt
    ...
    transformerInfoOption match {
      case Some(transformerInfo) => {
        env.mlModelBL.getSerializedTransformer(transformerInfo) match {
          ...
          case None =>
            throw new Exception(
              s"The file of transformaer model not found: name: $name, version: $version, timestamp: $timestampOpt"
            )
        }
      }
      ...
      case None =>
        throw new Exception(s"The model info not found: name: $name, version: $version, timestamp: $timestampOpt")
    }
  }

  /** It build a MlModelsBroadcastDB where all the model with info are broadcasted The param can be incomplete so it
    * read the complete model with info from MongoDB
  ...
  def write(mlModel: TransformerWithInfo): TransformerWithInfo = {
    val fileId: BsonObjectId =
      env.mlModelBL.saveTransformer(mlModel.transformer, mlModel.name, mlModel.version, mlModel.timestamp)
...
/** It allow to get a complete broadcasted model and keep a list of model to salve
  */
class MlModelsBroadcastDB(
    val modelDatabase: Map[String, Broadcast[TransformerWithInfo]] = Map(),
  ...
  ): Option[Broadcast[TransformerWithInfo]] = {
