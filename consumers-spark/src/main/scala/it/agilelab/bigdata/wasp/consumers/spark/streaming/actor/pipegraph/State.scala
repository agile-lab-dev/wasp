...
object State {
  ...
  /** [[PipegraphGuardian]] activated the pipegraph.
    *
    * @note
    *   it is a safe state with no in-flight request, so we can compute if we should retry activation or go to
    *   materializing
    */
  ...
  /** [[PipegraphGuardian]] materialized a pipegraph
    * @note
    *   it is a safe state with no in-flight request, so we can compute if we should retry materialization or go to
    *   Monitoring
    */
