...
object PartitionDiscoveryUtils {
  ...
  def listFiles(fs: FileSystem)(path: Path): List[Path] = {
    @tailrec
  ...
  def listDirectories(fs: FileSystem)(path: Path): List[Path] = {
    @tailrec
  ...
  /** Calculates the depth of the currentPath relative ot basePath
    */
  @tailrec
  final def fileDepth(fs: FileSystem)(basePath: Path, currentPath: Path, currDepth: Int = 0): Int = {
    require(
      fs.makeQualified(currentPath).toString.contains(fs.makeQualified(basePath).toString),
  ...
  /** Goes up to the passed number of level with regard to input p
    */
  @tailrec
  final private def goUp(p: Path, depth: Int): Path = {
    require(depth >= 0)
    if (depth == 0) {
      p
    } else {
  ...
  /** Discovers all the partitions columns and values of a given basePath
    *
    * @return
    *   a Left with an error message if something goes wrong during the partition discovery. a Right with a list of
    *   PartitionColumnValues. The list is ordered from the closest to the basePath column to the farthest.
    *   PartitionColumnValues are sorted alphabetically.
    */
  def discoverPartitions(fs: FileSystem)(basePath: Path): Either[String, List[PartitionColumnValues]] = {
    val files = listFiles(fs)(basePath).map(_.getParent).distinct
    Either
      .cond(files.nonEmpty, Nil, s"No files found under $basePath")
      .right
      .flatMap { _ =>
      ...
      .flatMap { depth =>
        ...
        Either.cond(
          columnAndValues.forall(_.isRight),
          columnAndValues.map(_.right.get),
          columnAndValues.find(_.isLeft).get.left.get
        )
      }
      .right
      .flatMap(x =>
  ...
  /** Discovers the column name and value of a given path at a given depth.
    *
    * @return
    *   a Left with an error message if something goes wrong during the value discovery. a Right with a list of column
    *   name and column value, this does not check that the layout is correct just that all the sub-folders have a
    *   compatible "name" (i.e. in the form of col=value)
    */
  ...
  /** Given a path returns column name and value of the leaf of that path, if any present enclosed in a Right. If any
    * error occurs, the returned value will be a Left enclosing an error description
    */
