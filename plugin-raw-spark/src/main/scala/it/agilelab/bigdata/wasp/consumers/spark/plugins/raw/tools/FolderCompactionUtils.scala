...
object FolderCompactionUtils {
  ...
  /** Generates all the possible combinations of columnName and columnValue Example: partitions = List( "a" ->
    * List("1"), "b" -> List("2", "3"), "c" -> List("4", "5", "6") ) output = List( ("a", "1") :: ("b", "2) :: ("c",
    * "4") :: Nil, ("a", "1") :: ("b", "2") :: ("c", "5") :: Nil, ("a", "1") :: ("b", "2") :: ("c", "6") :: Nil, ("a",
    * "1") :: ("b", "3") :: ("c", "4") :: Nil, ("a", "1") :: ("b", "3") :: ("c", "5") :: Nil, ("a", "1") :: ("b", "3")
    * :: ("c", "6") :: Nil, )
    * @param partitions
  ...
  private def generateCombinationsForValues(
      col: ColumnName,
      values: List[ColumnValue],
      others: List[(ColumnName, List[ColumnValue])]
  ): List[PartitionsCombination] = {
    values.flatMap(value => generateCombinationsForValue(col, value, others))
  }

  private def generateCombinationsForValue(
      col: String,
      value: String,
      others: List[(ColumnName, List[ColumnValue])]
  ): List[PartitionsCombination] = {
    ...
    conf.getValue(key).valueType() match {
      ...
      case _ =>
        throw new IllegalArgumentException(
          s"Conf: ${conf.getValue(key)} is neither a String or " +
            s"a configuration"
        )
    }
  }
    ...
    val options = {
      ...
      val extraOptions =
        if (conf.hasPath("options.extraOptions")) {
          Some(
            conf
              .getConfig("options.extraOptions")
              .entrySet()
              .asScala
              .map { entry =>
  ...
  /** Builds the list of WhereCondition used to filter the original DataFrame read from the input model. This list has
    * one element for each output partition combination, in order to write the correct number of files to the partitions
    * specified by the output model. Each of these combinations is put in AND with all the input partitions
    * combinations, in order to write only the files of the partitions requested. Example: inputModel.partitions = [a,
    * b, c, d] outputModel.partitions = [a, b] partitions = a -> [1, 2], b -> [3, 4], c -> [5, 6], d -> [7, 8]
    *
    * output = [ (a=1 AND b=3) AND ( (c=5 AND d=7) OR (c=5 AND d=8) OR (c=6 AND d=7) OR (c=6 AND d=8) ) (a=1 AND b=4)
    * AND ( (c=5 AND d=7) OR (c=5 AND d=8) OR (c=6 AND d=7) OR (c=6 AND d=8) ) (a=2 AND b=3) AND ( (c=5 AND d=7) OR (c=5
    * AND d=8) OR (c=6 AND d=7) OR (c=6 AND d=8) ) (a=2 AND b=4) AND ( (c=5 AND d=7) OR (c=5 AND d=8) OR (c=6 AND d=7)
    * OR (c=6 AND d=8) ) ]
    * @param partitions
    ...
    */
  def generateWhereConditions(
      partitions: Map[ColumnName, List[ColumnValue]],
      inputModel: RawModel,
    ...
    val outputPartitionsCombinations: List[PartitionsCombination] = generateCombinations(
      partitions.filter(x => commonColumns.contains(x._1)).toList
    )
    val inputOnlyPartitionsCombinations: List[PartitionsCombination] = generateCombinations(
      partitions.filter(x => onlyInputColumns.contains(x._1)).toList
    )
  ...
  /** @param files
    ...
    */
  def filterWhereCondition(files: List[Path])(whereCondition: WhereCondition): Boolean = {
  ...
  /** @param whereCondition
    ...
    */
  def filterSingleQuery(whereCondition: WhereCondition)(file: Path): Boolean = {
    ...
    val secondAnd = whereCondition.inPartitionsCombinations match {
      case Nil => true
      case x =>
        x.foldLeft(false) { case (acc, combination) =>
          acc || filterPartitionsCombination(folders, combination)
        }
    }
  ...
  /** @param folders
    ...
    */
  def filterPartitionsCombination(folders: Array[String], combination: PartitionsCombination): Boolean = {
  ...
  /** @param path
    ...
    */
  def filterPath(path: Path)(combinations: List[String]): Boolean = {
