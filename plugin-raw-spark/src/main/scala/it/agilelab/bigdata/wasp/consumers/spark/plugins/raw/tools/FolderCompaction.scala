...
class FolderCompaction extends Logging {
  ...
  /** Receives the following conf: { "inputModel" : "name of the input model" | ModelConf, "outputModel" : "name of the
    * input model" | ModelConf, "partitions" : { "String1" : ["value1", "value2"], "String2" : ["value1", "value2"] },
    * "numPartitions" : "integer" }
    *
    * ModelConf has the following structure:
    *
    * { "name": "a name you like" "uri": "URI of the dataset: basePath if the dataset is partitioned" "schema": "Spark
    * json representation of the schema" "timed": true/false "options": { saveMode: "spark save mode" format: "spark
    * data format" extraOptions: { // "extra format to the spark reader/writer" key: value } partitionBy: [ //
    * "partition columns" partitionColumn1, partitionColumn2, ... partitionColumnN ] } }
    *
    * The function will retrieve or build the two indicated models, check that they are file-based, and check that the
    * indicated columns are partition columns. If the previous requirements are met then it will generate each
    * combination of column values and then perform read, repartition to the "numPartitions" number, write to the
    * outputModel and delete the files read.
    *
    * @param conf
    *   the Config
    * @param spark
  ...
  /** Deletes all files that have been read and all empty folders after
    ...
    */
  def delete(spark: SparkSession, rootDir: String, dfs: List[DataFrame]): Unit = {
    dfs match {
      case Nil =>
      ...
      case hDf :: tailDfs =>
        ...
        files match {
          ...
          case head :: _ =>
            ...
            val deletedEmptyFolders =
              deleteEmptyPartitionFolders(fs, fs.makeQualified(new Path(rootDir)), pathsToDelete)
  ...
  def read(
    ...
    val filteredPartitions = partitions.filter { case (colName, _) =>
      inputModel.options.partitionBy.exists(_.contains(colName))
    }
    ...
    val filteredWhereConditions: List[Column] = whereConditions
      .filter(filterWhereCondition(inputModelFiles))
      .map(_.toSparkColumn)
    logger.info(s"Where conditions: $filteredWhereConditions")

    // apply whereConditions to the read DataFrame
    val loadedDataFrames = filteredWhereConditions.map { query =>
      unfilteredDF.where(query)
    }
