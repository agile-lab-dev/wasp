...
trait ConfigurationSupport {
  ...
  def parse(sparkConfig: SparkConf): NifiPluginConfig = {
    val hadoopConfig = CompatibilitySparkHadoopUtil.get.newConfiguration(sparkConfig)
...
class NifiPlugin(sparkConf: SparkConf)
    extends CompatibilityNifiPlugin
  ...
  protected val config: NifiPluginConfig = parse(sparkConf)
