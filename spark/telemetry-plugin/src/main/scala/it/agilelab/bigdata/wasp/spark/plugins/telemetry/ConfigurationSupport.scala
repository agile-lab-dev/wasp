...
trait ConfigurationSupport {
  ...
  private def parseTelemetry() = {
    val telemetryConfigJSON =
      new String(Base64.getUrlDecoder.decode(System.getProperty("wasp.plugin.telemetry.topic")), StandardCharsets.UTF_8)
    ...
    val jmx = tt
      ...
      .map(_.map { s =>
        TelemetryPluginJMXTelemetryConfigModel(
          query = s("query").asInstanceOf[String],
  ...
  private def parseKafka() = {
    val kafkaTinyConfigJSON =
      new String(Base64.getUrlDecoder.decode(System.getProperty("wasp.plugin.telemetry.kafka")), StandardCharsets.UTF_8)
