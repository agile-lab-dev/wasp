...
echo  "Options WASP_CONFIGURATION_FILE: '$WASP_CONFIGURATION_FILE', DROP_MONGODB_OPT: $DROP_MONGODB_OPT, WASP_SECURITY: $WASP_SECURITY, WASP_YARN: $WASP_YARN, WASP_LAUNCHER_OPTS: $WASP_LAUNCHER_OPTS"
...
DOCKER_OPTS="-i -v $SCRIPT_DIR:/root/configurations --network=wasp-docker --rm -w /root/configurations/"
DOCKER_IMAGE="agilefactory/oracle-java:jdk-8u162"

SECURITY_DOCKER_OPTS=""
if [ -n "$WASP_SECURITY" ]; then
   echo "Enable security in wasp"
...
if [ -n "$WASP_YARN" ]; then
   DOCKER_OPTS="$DOCKER_OPTS -v $SCRIPT_DIR/docker-service-configuration/yarn-conf:/etc/hadoop/conf/ "
else
   DOCKER_OPTS="$DOCKER_OPTS -v $SCRIPT_DIR/docker-service-configuration/hdfs:/etc/hadoop/conf/ -v $SCRIPT_DIR/docker-service-configuration/hbase:/etc/hbase/conf/"
fi

if [ -n "$DROP_MONGODB_OPT" ]; then
    echo "Running module 'master' in container in order to drop MongoDB database"
    $DOCKER_CMD run ${DOCKER_OPTS} ${SECURITY_DOCKER_OPTS} \
    --name master -p 2891:2891 -p 5005:5005 \
    -v "$MASTER_PROJECT_DIRECTORY/target/universal/stage/":/root/wasp/ \
    ${DOCKER_IMAGE} /root/configurations/entrypoint.sh ${MASTER_PROJECT_COMMAND} ${WASP_OPTS} -jvm-debug 5005 \
...
$DOCKER_CMD run ${DOCKER_OPTS} --name producers \
    -p 5006:5006 \
    ${SECURITY_DOCKER_OPTS} \
    -v "$PRODUCERS_PROJECT_DIRECTORY/target/universal/stage/":/root/wasp/ \
...
$DOCKER_CMD run ${DOCKER_OPTS} --name consumers-rt \
    -p 5007:5007 \
    ${SECURITY_DOCKER_OPTS} \
    -v "$CONSUMERS_RT_PROJECT_DIRECTORY/target/universal/stage/":/root/wasp/ \
    ${DOCKER_IMAGE} /root/configurations/entrypoint.sh ${CONSUMERS_RT_PROJECT_COMMAND} ${WASP_OPTS} -jvm-debug 5007 \
...
$DOCKER_CMD run ${DOCKER_OPTS} --name consumers-spark-streaming \
    ...
    -v "$CONSUMERS_SPARK_PROJECT_DIRECTORY/target/universal/stage/":/root/wasp/ \
    ${DOCKER_IMAGE} /root/configurations/entrypoint.sh ${CONSUMERS_SPARK_PROJECT_COMMAND} -main ${CONSUMERS_SPARK_STREAMING_MAIN_CLASS} ${WASP_OPTS} \
    ...
    ${WASP_LAUNCHER_OPTS} 2>&1              | sed "s/.*/$b consumers-spark  |$d &/" &
$DOCKER_CMD run ${DOCKER_OPTS} --name consumers-spark-batch \
    -p 4039:4040 -p 5009:5009 -p 31441:31441 -p 31442:31442 \
    ${SECURITY_DOCKER_OPTS} \
     -e HADOOP_CONF_DIR=/etc/hadoop/conf/   \
    -v "$CONSUMERS_SPARK_PROJECT_DIRECTORY/target/universal/stage/":/root/wasp/ \
    ${DOCKER_IMAGE} /root/configurations/entrypoint.sh ${CONSUMERS_SPARK_PROJECT_COMMAND} -main ${CONSUMERS_SPARK_BATCH_MAIN_CLASS} ${WASP_OPTS} \
