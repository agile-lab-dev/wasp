...
case class IndustrialPlantData(
    site: String,
    plant: String,
    line: String,
    machine: String,
...
object FakeIndustrialPlantData {
  ...
  def fromRandom(): IndustrialPlantData = {
    ...
    IndustrialPlantData(
      sites(random.nextInt(5)),
      plants(random.nextInt(6)),
      lines(random.nextInt(5)),
      machines(random.nextInt(6)),
      areadIds(random.nextInt(6)),
      time,
      kpi(kpiIndex)._1,
      getKpiValue(kpiIndex)
    )
  }
}
