...
abstract class TestCheckpointStrategy(val version: String, val topicName: String) extends Strategy {
  ...
  override def transform(dataFrames: Map[ReaderKey, DataFrame]): DataFrame = {
    ...
    groupedDataset
      .flatMapGroupsWithState[TestCheckpointState, TestCheckpointDocument](
        OutputMode.Append,
  ...
  def update
      : (String, Iterator[TestCheckpointDocument], GroupState[TestCheckpointState]) => Iterator[TestCheckpointDocument]
}
...
class TestCheckpointJSONStrategyV1 extends TestCheckpointStrategy("v1", TestTopicModel.jsonCheckpoint.name) {
  ...
  override def update =
    (key: String, iterator: Iterator[TestCheckpointDocument], state: GroupState[TestCheckpointState]) => {
      iterator map { element =>
        ...
        println(
          s"Element: ${element.id} - Version: ${element.version} - Value: ${element.value} - OldState: $oldState - NewState: ${state.get}"
        )
      ...
      iterator map { element =>
        ...
        state.update(
          oldState match {
            case oldStateImpl: TestCheckpointStateV3 => {
              ...
              TestCheckpointStateV3(
                sum = oldStateImpl.sum + element.value,
                oldSumInt = oldStateImpl.sum,
                oldSumString = oldStateImpl.sum.toString
              )
            }
            case oldStateImpl: TestCheckpointStateV2 => {
              // do something using oldStateImpl TestCheckpointStateV2
        ...
        state.get match {
          case newStateImpl: TestCheckpointStateV3 => {
            ...
            element.copy(
              sum = newStateImpl.sum,
              oldSumInt = newStateImpl.oldSumInt,
              oldSumString = newStateImpl.oldSumString
            )
          }
          case newStateImpl => {
            // do nothing using newStateImpl
      ...
      iterator map { element =>
        ...
        state.update(
          oldState match {
            case oldStateImpl: TestCheckpointStateV4 => {
              ...
              TestCheckpointStateV4(
                sum = oldStateImpl.sum + element.value,
                stateV4Internal =
                  TestCheckpointStateV4Internal(oldSumInt = oldStateImpl.sum, oldSumString = oldStateImpl.sum.toString)
              )
            }
            case oldStateImpl: TestCheckpointStateV3 => {
              // do something using oldStateImpl TestCheckpointStateV3
        ...
        state.get match {
          case newStateImpl: TestCheckpointStateV4 => {
            ...
            element.copy(
              sum = newStateImpl.sum,
              oldSumInt = newStateImpl.stateV4Internal.oldSumInt,
              oldSumString = newStateImpl.stateV4Internal.oldSumString
            )
          }
          case newStateImpl => {
            // do nothing using newStateImpl
