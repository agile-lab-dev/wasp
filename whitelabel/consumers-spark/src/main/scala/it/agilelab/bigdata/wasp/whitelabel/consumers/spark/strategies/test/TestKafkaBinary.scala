...
/** A simple strategy that adds columns for key, value, headers and destination topic for testing.
  ...
  *   Nicolò Bidotti
  */
class TestKafkaBinary extends Strategy {
  ...
  override def transform(dataFrames: Map[ReaderKey, DataFrame]): DataFrame = {
    ...
    df.withColumn("myKey", lit("messageKey"))
      .withColumn("myValue", lit("messageValue".getBytes))
      .withColumn(
        "myHeaders",
        array(
          struct(lit("key").as("headerKey"), lit("value".getBytes).as("headerValue")),
          struct(lit("key2").as("headerKey"), lit("value2".getBytes).as("headerValue"))
        )
      )
      .withColumn("topics", array(lit("test_binary.topic"), lit("test2_binary.topic")))
