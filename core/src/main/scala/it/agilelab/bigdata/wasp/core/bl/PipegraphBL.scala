...
trait PipegraphBL  {
  ...
  def setIsActive(pipegraphModel: PipegraphModel, isActive: Boolean): Unit = {
    ...
    pipegraphModel.streaming.foreach(etl => etl.isActive = isActive)
...
class PipegraphBLImp(waspDB: WaspDB) extends PipegraphBL {
  ...
  private def factory(p: PipegraphModel) = PipegraphModel(p.name, p.description, p.owner, p.isSystem, p.creationTime, p.streaming, p.structuredStreaming, p.rt, p.dashboard, p.isActive, p._id)
