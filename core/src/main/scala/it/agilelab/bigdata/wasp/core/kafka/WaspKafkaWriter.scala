...
import it.agilelab.bigdata.wasp.core.models.configuration.{KafkaConfigModel, KafkaEntryConfigModel, TinyKafkaConfig}
...
object WaspKafkaWriter {
  ...
  def createConfig(brokers: Set[String], batchSize: Int,  keySerializerFqcn: String, serializerFqcn: String,
                   otherConfigs: Seq[KafkaEntryConfigModel] = Seq()): Properties = {
    ...
    props.put("acks", otherConfigs.filter(_.key == "acks").map(_.value).headOption.getOrElse("1"))

    otherConfigs.foreach(v => {
