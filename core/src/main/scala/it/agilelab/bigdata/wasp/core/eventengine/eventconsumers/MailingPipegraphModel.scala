...
object MailingPipegraphModel {
  ...
  private val mailingPipegraphSettings: MailingPipegraphSettings =
    MailingPipegraphSettingsFactory.create(ConfigFactory.load())
  ...
  private val mailerWriterModel: WriterModel = maybeWriterSettings match {
    case Some(writerSettings) =>
      WriterModel.webMailWriter(
        writerSettings.modelName,
        WebMailModel(writerSettings.dataStoreModelName),
        sanitizedOptions
      )
    ...
    strategiesSettings
      .map(s => {
        StructuredStreamingETLModel(
          ...
          strategy =
            Some(StrategyModel.create("it.agilelab.bigdata.wasp.consumers.spark.eventengine.MailStrategy", s.trigger)),
          triggerIntervalMs =
            if (s.triggerIntervalMs.isDefined) s.triggerIntervalMs
            else mailingPipegraphSettings.defaultTriggerIntervalMs
        )
      })
      .toList
