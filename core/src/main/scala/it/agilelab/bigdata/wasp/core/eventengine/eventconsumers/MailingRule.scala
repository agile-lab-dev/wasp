...
/** Mailing rules store information about how to craft and send an e-mail. Mailing rules store an SQL statement which
  * indicates whether or not an e-mail should be sent for the specific Event, and another statement to enrich it with
  * mail subject. In the mailing rules are also specified mail recipient info to use, and the path to the velocity
  * template to use to compose the e-mail.
  *
  * @param mailingRuleName
  *   is the name of the MailingRule
  * @param ruleStatement
  *   defines whether a Mail should be sent or not
  * @param subjectStatement
  *   enrich the mail with a subject. WARNING: this statement is ignored if the mail aggregation options is enabled.
  *   FIXME: avoid
  * @param templatePath
