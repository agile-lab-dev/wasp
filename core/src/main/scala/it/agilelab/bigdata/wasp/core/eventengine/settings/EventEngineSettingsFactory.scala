  ...
  def create(config: Config): EventPipegraphSettings = {
    ...
    if (config.hasPath(keyword)) {
      ...
      val eventStrategyList: Seq[EventProducerETLSettings] =
        config
          .getConfigList(prefix + EVENT_STRATEGY)
          .asScala
          .map(cfg => {
            val name = cfg.getString(NAME)

            val triggerIntervalMs: Option[Long] =
              if (config.hasPath(prefix + TRIGGER_INTERVAL_MS)) Some(config.getInt(prefix + TRIGGER_INTERVAL_MS))
              else None
      ...
      EventPipegraphSettings(eventStrategies = Seq(), defaultTriggerIntervalMs = None, isSystem = false)
    }
  ...
  def create(config: Config): MailingPipegraphSettings = {
    ...
    if (config.hasPath(keyword)) {
      ...
      val mailingStrategyList: Seq[EventConsumerETLSettings] =
        config
          .getConfigList(prefix + MAILING_STRATEGY)
          .asScala
          .map(cfg => {
            val name = cfg.getString(NAME)
...
object EventStrategySettingsFactory {
  ...
  def create(config: Config): EventStrategySettings = {
    ...
    val rules: Seq[EventRule] = config
      .getObjectList(EVENT_RULES)
      .asScala
      .map(elem => {
...
object MailingStrategySettingsFactory {
  def create(config: Config): MailingStrategySettings = {
    ...
    val rules: Seq[MailingRule] = config
      .getObjectList(MAIL_RULES)
      .asScala
      .map(elem => {
        val cfg = elem.toConfig
    ...
    MailingStrategySettings(rules = rules, enableMailAggregation = enableAggregation)
  }
}
