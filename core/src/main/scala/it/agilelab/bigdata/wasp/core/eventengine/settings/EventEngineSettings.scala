...
case class EventPipegraphSettings(
    eventStrategies: Seq[EventProducerETLSettings],
    defaultTriggerIntervalMs: Option[Long],
    isSystem: Boolean
)

case class MailingPipegraphSettings(
    mailWriterSettings: Option[ModelSettings],
    defaultTriggerIntervalMs: Option[Long],
    mailingStrategies: Seq[EventConsumerETLSettings],
    isSystem: Boolean
)

/** Event producer ETL settings defines the Event generation flow. An [[EventProducerETLSettings]] object refers to a
  * single [[StructuredStreamingETLModel]].
  *
  ...
  */
case class EventProducerETLSettings(
    name: String,
...
/** Event consumer ETL settings defines the Event consumption flow. A [[EventConsumerETLSettings]] objects refers to a
  * single [[StructuredStreamingETLModel]].
  *
  * @param name
  *   is the name of the MailingStrategy to create
  * @param readerModel
  *   is the information about where to fetch Event objects No writerModel because mail pipegraph is a funnel, so it's
  *   defined at pipegraph configuration level
  * @param trigger
  *   is the Typesafe config object which defines mail rules
  */
case class EventConsumerETLSettings(
    name: String,
    triggerIntervalMs: Option[Long],
    readerModel: ModelSettings,
    trigger: Config
...
/** Defines settings for a single MailingStrategy
  ...
  * @param enableMailAggregation:
  *   if true mails with same recipient will be aggregated in a single Mail
  */
