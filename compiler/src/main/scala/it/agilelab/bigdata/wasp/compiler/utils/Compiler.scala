...
class Compiler {
    ...
    try {
      ...
      case e: Throwable =>
        throw new RuntimeException(
          "Unable lo load scala interpreter from classpath (scala-compiler jar is missing?)",
          e
        )
    }
  ...
  private lazy val impliedClassPath: List[String] = {
    val currentClassPath: List[String] = this.getClass.getClassLoader
      .asInstanceOf[URLClassLoader]
      .getURLs
      .map(_.toString)
      .filter(_.startsWith("file:"))
      .filterNot(_.contains("jaxb"))
      .map(_.substring(5))
      .toList
    ...
    currentClassPath ::: (if (currentClassPath.size == 1 && currentClassPath.head.endsWith(".jar")) {
                            ...
                            val nestedClassPath =
                              new JarFile(jarFile).getManifest.getMainAttributes.getValue("Class-Path")
