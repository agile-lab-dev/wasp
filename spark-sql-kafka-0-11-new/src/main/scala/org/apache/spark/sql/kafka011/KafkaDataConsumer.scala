...
import org.apache.spark.sql.kafka011.KafkaDataConsumer.AvailableOffsetRange
...
sealed private[kafka011] trait KafkaDataConsumer {
...
/**
  ...
  */
private[kafka011] case class InternalKafkaConsumer(topicPartition: TopicPartition, kafkaParams: ju.Map[String, Object])
    extends Logging {
  ...
  /**
    ...
    * instead.
    */
  private case class FetchedRecord(var record: ConsumerRecord[Array[Byte], Array[Byte]], var nextOffsetToFetch: Long) {

    def withRecord(record: ConsumerRecord[Array[Byte], Array[Byte]], nextOffsetToFetch: Long): FetchedRecord = {
      this.record = record
  ...
  private def runUninterruptiblyIfPossible[T](body: => T): T = Thread.currentThread match {
    ...
    case _ =>
      logWarning(
        "CachedKafkaConsumer is not running in UninterruptibleThread. " +
  ...
  private def getEarliestAvailableOffsetBetween(offset: Long, untilOffset: Long): Long = {
    ...
    if (offset >= range.latest || range.earliest >= untilOffset) {
      ...
      logWarning(
        s"Found a disappeared offset $offset. " +
  ...
  private def fetchRecord(
    ...
    if (!fetchedData.hasNext) {
      ...
      if (record.offset > offset) {
        ...
        // mechanism.
        throw new IllegalStateException(s"Tried to fetch $offset but the returned record offset was ${record.offset}")
      } else {
        fetchedRecord.withRecord(record, fetchedData.nextOffsetInFetchedData)
      }
    }
  ...
  /**
    ...
    */
  private def reportDataLoss(failOnDataLoss: Boolean, message: String, cause: Throwable = null): Unit = {
  ...
  private def fetchData(offset: Long, pollTimeoutMs: Long): Unit = {
    ...
    if (!fetchedData.hasNext) {
      ...
      if (offset < range.earliest || offset >= range.latest) {
        throw new OffsetOutOfRangeException(Map(topicPartition -> java.lang.Long.valueOf(offset)).asJava)
      } else if (offset == offsetAfterPoll) {
        throw new TimeoutException(s"Cannot fetch record for offset $offset in $pollTimeoutMs milliseconds")
      }
    }
  }
}

private[kafka011] object KafkaDataConsumer extends Logging {

  case class AvailableOffsetRange(earliest: Long, latest: Long)

  private case class CachedKafkaDataConsumer(internalConsumer: InternalKafkaConsumer) extends KafkaDataConsumer {
  ...
  private case class NonCachedKafkaDataConsumer(internalConsumer: InternalKafkaConsumer) extends KafkaDataConsumer {
  ...
  private lazy val cache = {
    ...
    new ju.LinkedHashMap[CacheKey, InternalKafkaConsumer](capacity, 0.75f, true) {
      override def removeEldestEntry(entry: ju.Map.Entry[CacheKey, InternalKafkaConsumer]): Boolean = {
  ...
  def acquire(
      ...
      useCache: Boolean
  ): KafkaDataConsumer = synchronized {
...
private[kafka011] object InternalKafkaConsumer extends Logging {

  private val UNKNOWN_OFFSET = -2L

  private def reportDataLoss0(failOnDataLoss: Boolean, finalMessage: String, cause: Throwable = null): Unit = {
    if (failOnDataLoss) {
      if (cause != null) {
