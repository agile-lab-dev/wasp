...
class KafkaContinuousSinkSuite extends KafkaContinuousTest {
  ...
  test("streaming - write w/o topic field, with topic option") {
    ...
    val writer = createKafkaWriter(input.toDF(), withTopic = Some(topic), withOutputMode = Some(OutputMode.Append()))()
  ...
  test("null topic attribute") {
    ...
    try {
      writer = createKafkaWriter(input.toDF())(
        withSelectExpr = "CAST(null as STRING) as topic",
        "value"
      )
      testUtils.sendMessages(inputTopic, Array("1", "2", "3", "4", "5"))
      eventually(timeout(streamingTimeout)) {
  ...
  test("streaming - exception on config serializer") {
    ...
    try {
      ...
      assert(ex.getMessage.toLowerCase(Locale.ROOT).contains("kafka option 'key.serializer' is not supported"))
    } finally {
      writer.stop()
    }
    ...
    try {
      ...
      assert(ex.getMessage.toLowerCase(Locale.ROOT).contains("kafka option 'value.serializer' is not supported"))
    } finally {
      writer.stop()
    }
  }
  ...
  test("generic - write big data with small producer buffer") {
    ...
    val writeTask =
      new KafkaStreamDataWriter(Some(topic), options.asInstanceOf[java.util.Map[String, AnyRef]], inputSchema)
    try {
