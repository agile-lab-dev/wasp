...
class KafkaOffsetRangeCalculatorSuite extends SparkFunSuite {
  ...
  def testWithMinPartitions(name: String, minPartition: Int)(f: KafkaOffsetRangeCalculator => Unit): Unit = {
  ...
  test("with no minPartition: N TopicPartitions to N offset ranges") {
    ...
    assert(
      calc.getRanges(fromOffsets = Map(tp1 -> 1), untilOffsets = Map(tp1 -> 2)) ==
        Seq(KafkaOffsetRange(tp1, 1, 2, None))
    )

    assert(
      calc.getRanges(fromOffsets = Map(tp1 -> 1), untilOffsets = Map(tp1 -> 2, tp2 -> 1), Seq.empty) ==
        Seq(KafkaOffsetRange(tp1, 1, 2, None))
    )

    assert(
      calc.getRanges(fromOffsets = Map(tp1 -> 1, tp2 -> 1), untilOffsets = Map(tp1 -> 2)) ==
        Seq(KafkaOffsetRange(tp1, 1, 2, None))
    )
  ...
  test("with no minPartition: empty ranges ignored") {
    val calc = KafkaOffsetRangeCalculator(DataSourceOptions.empty())
    assert(
      calc.getRanges(fromOffsets = Map(tp1 -> 1, tp2 -> 1), untilOffsets = Map(tp1 -> 2, tp2 -> 1)) ==
        Seq(KafkaOffsetRange(tp1, 1, 2, None))
    )
  }
  testWithMinPartitions("N TopicPartitions to N offset ranges with executors", 3) { calc =>
    assert(
      calc.getRanges(
        Map(tp1 -> 1, tp2 -> 1, tp3 -> 1),
        Map(tp1 -> 2, tp2 -> 2, tp3 -> 2),
  ...
  testWithMinPartitions("N TopicPartitions to N offset ranges", 3) { calc =>
    assert(
      calc
        .getRanges(fromOffsets = Map(tp1 -> 1, tp2 -> 1, tp3 -> 1), untilOffsets = Map(tp1 -> 2, tp2 -> 2, tp3 -> 2)) ==
        Seq(KafkaOffsetRange(tp1, 1, 2, None), KafkaOffsetRange(tp2, 1, 2, None), KafkaOffsetRange(tp3, 1, 2, None))
    )
  }

  testWithMinPartitions("1 TopicPartition to N offset ranges", 4) { calc =>
    assert(
      calc.getRanges(fromOffsets = Map(tp1 -> 1), untilOffsets = Map(tp1 -> 5)) ==
        Seq(
    ...
    assert(
      calc.getRanges(fromOffsets = Map(tp1 -> 1), untilOffsets = Map(tp1 -> 5), executorLocations = Seq("location")) ==
        Seq(
          KafkaOffsetRange(tp1, 1, 2, None),
          KafkaOffsetRange(tp1, 2, 3, None),
          KafkaOffsetRange(tp1, 3, 4, None),
  ...
  testWithMinPartitions("N skewed TopicPartitions to M offset ranges", 3) { calc =>
    assert(
      calc.getRanges(fromOffsets = Map(tp1 -> 1, tp2 -> 1), untilOffsets = Map(tp1 -> 5, tp2 -> 21)) ==
        Seq(
  ...
  testWithMinPartitions("range inexact multiple of minPartitions", 3) { calc =>
    assert(
      calc.getRanges(fromOffsets = Map(tp1 -> 1), untilOffsets = Map(tp1 -> 11)) ==
        Seq(KafkaOffsetRange(tp1, 1, 4, None), KafkaOffsetRange(tp1, 4, 7, None), KafkaOffsetRange(tp1, 7, 11, None))
    )
  }

  testWithMinPartitions("empty ranges ignored", 3) { calc =>
    assert(
      calc.getRanges(fromOffsets = Map(tp1 -> 1, tp2 -> 1, tp3 -> 1), untilOffsets = Map(tp1 -> 5, tp2 -> 21, tp3 -> 1)) ==
        Seq(
          KafkaOffsetRange(tp1, 1, 5, None),
          KafkaOffsetRange(tp2, 1, 7, None),
          KafkaOffsetRange(tp2, 7, 14, None),
