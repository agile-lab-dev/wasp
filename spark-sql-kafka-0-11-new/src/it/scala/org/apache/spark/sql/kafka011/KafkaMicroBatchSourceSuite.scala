...
import org.apache.spark.sql.kafka011.KafkaSourceProvider._
...
abstract class KafkaSourceTest extends StreamTest with SharedSQLContext with KafkaTest {
  ...
  case class AddKafkaData(topics: Set[String], data: Int*)(
      ...
      message: String = "",
      topicAction: (String, Option[Int]) => Unit = (_, _) => {}
  ) extends AddData {
    ...
    override def addData(query: Option[StreamExecution]): (BaseStreamingSource, Offset) = {
      ...
      require(query.nonEmpty, "Cannot add data when there is no query for finding the active kafka source")
      ...
      if (sources.isEmpty) {
        throw new Exception("Could not find Kafka source in the StreamExecution logical plan to add data to")
  ...
  object WithOffsetSync {
    ...
    /**
      ...
      */
    def apply(topicPartition: TopicPartition, expectedOffset: Long)(func: () => Unit): StreamAction = {
...
abstract class KafkaMicroBatchSourceSuiteBase extends KafkaSourceSuiteBase {
  ...
  test(
    "SPARK-26718 Rate limit set to Long.Max should not overflow integer " +
  ...
  test("maxOffsetsPerTrigger") {
    ...
    testStream(mapped)(
      ...
      CheckAnswer(1, 10, 100, 101, 102, 103, 104, 105, 106, 107, 11, 108, 109, 110, 111, 112, 113, 114, 115, 116),
      StopStream,
      StartStream(ProcessingTime(100), clock),
      waitUntilBatchProcessed,
      // smallest now empty, 1 more from middle, 9 more from biggest
      CheckAnswer(1, 10, 100, 101, 102, 103, 104, 105, 106, 107, 11, 108, 109, 110, 111, 112, 113, 114, 115, 116, 12,
        117, 118, 119, 120, 121, 122, 123, 124, 125),
      AdvanceManualClock(100),
      waitUntilBatchProcessed,
      // smallest now empty, 1 more from middle, 9 more from biggest
      CheckAnswer(1, 10, 100, 101, 102, 103, 104, 105, 106, 107, 11, 108, 109, 110, 111, 112, 113, 114, 115, 116, 12,
        117, 118, 119, 120, 121, 122, 123, 124, 125, 13, 126, 127, 128, 129, 130, 131, 132, 133, 134)
    )
  }
  ...
  test("ensure that initial offset are written with an extra byte in the beginning (SPARK-19517)") {
    withTempDir { metadataPath =>
      ...
      testStream(df)(StartStream(checkpointLocation = metadataPath.getAbsolutePath), makeSureGetOffsetCalled)
  ...
  test("deserialization of initial offset written by Spark 2.1.0 (SPARK-19517)") {
    withTempDir { metadataPath =>
      ...
      val from = getClass.getClassLoader.getResourceAsStream("kafka-source-initial-offset-version-2.1.0.bin")
  ...
  test("deserialization of initial offset written by future version") {
    withTempDir { metadataPath =>
      ...
      // Copy the initial offset file into the right location inside the checkpoint root directory
      // such that the Kafka source can read it for initial offsets.
      val from = getClass.getClassLoader.getResourceAsStream("kafka-source-initial-offset-future-version.bin")
      val to   = Paths.get(s"${metadataPath.getAbsolutePath}/sources/0/0")
      Files.createDirectories(to.getParent)
      Files.copy(from, to)
  ...
  test("KafkaSource with watermark") {
    ...
    // conversion works.
    assert(row.getAs[java.sql.Timestamp]("window").getTime >= now - 5 * 1000, s"Unexpected results: $row")
  ...
  test("ensure stream-stream self-join generates only one offset in log and correct metrics") {
    ...
    val values = kafka
      .selectExpr("CAST(CAST(value AS STRING) AS INT) AS value", "CAST(CAST(value AS STRING) AS INT) % 5 AS key")
...
class KafkaMicroBatchV1SourceSuite extends KafkaMicroBatchSourceSuiteBase {
  override def beforeAll(): Unit = {
    super.beforeAll()
    spark.conf.set("spark.sql.streaming.disabledV2MicroBatchReaders", classOf[KafkaSourceProvider].getCanonicalName)
  }
...
class KafkaMicroBatchV2SourceSuite extends KafkaMicroBatchSourceSuiteBase {
  ...
  testWithUninterruptibleThread("minPartitions is supported") {
    ...
    def test(minPartitions: String, numPartitionsGenerated: Int, reusesConsumers: Boolean): Unit = {
      ...
      withTempDir { dir =>
        ...
        withClue(s"minPartitions = $minPartitions generated factories $factories\n\t") {
          ...
          factories.foreach { f =>
            assert(f.reuseKafkaConsumer == reusesConsumers)
          }
        }
      }
    }
...
abstract class KafkaSourceSuiteBase extends KafkaSourceTest {
  ...
  for (failOnDataLoss <- Seq(true, false)) {
    ...
    test(s"subscribing topic by name from latest offsets (failOnDataLoss: $failOnDataLoss)") {
      val topic = newTopic()
      testFromLatestOffsets(topic, addPartitions = true, failOnDataLoss = failOnDataLoss, "subscribe" -> topic)
    }

    test(s"subscribing topic by name from earliest offsets (failOnDataLoss: $failOnDataLoss)") {
      val topic = newTopic()
      testFromEarliestOffsets(topic, addPartitions = true, failOnDataLoss = failOnDataLoss, "subscribe" -> topic)
    }
    ...
    test(s"subscribing topic by pattern from specific offsets (failOnDataLoss: $failOnDataLoss)") {
      val topicPrefix = newTopic()
      val topic       = topicPrefix + "-suffix"
      testFromSpecificOffsets(topic, failOnDataLoss = failOnDataLoss, "subscribePattern" -> s"$topicPrefix-.*")
    }
  }
  ...
  test("bad source options") {
    ...
    testBadOptions("subscribe" -> "t", "subscribePattern" -> "t.*")("only one", "options can be specified")

    testBadOptions("subscribe" -> "t", "assign" -> """{"a":[0]}""")("only one", "options can be specified")
  ...
  private def testFromSpecificOffsets(topic: String, failOnDataLoss: Boolean, options: (String, String)*): Unit = {
    val partitionOffsets = Map(
    ...
    testStream(mapped)(
      ...
      Execute { q =>
        ...
        q.awaitOffset(0, KafkaSourceOffset(partitionOffsets.mapValues(_ => 3L)), streamingTimeout.toMillis)
      },
  ...
  test("Kafka column types") {
    ...
    val topic = newTopic()
    testUtils.createTopic(newTopic(), partitions = 1)
    val record = new ProducerRecord(topic, "1", "1")
    record.headers().add("a", "b".getBytes(UTF_8)).add("c", "d".getBytes(UTF_8))
    testUtils.sendMessages(Array(record))
    ...
    eventually(timeout(streamingTimeout)) {
      assert(
        spark.table("kafkaColumnTypes").count == 1,
        s"Unexpected results: ${spark.table("kafkaColumnTypes").collectAsList()}"
      )
    }
    val row = spark.table("kafkaColumnTypes").head()
    assert(row.getAs[Array[Byte]]("key") === "1".getBytes(UTF_8), s"Unexpected results: $row")
    ...
    def checkHeader(row: Row, expected: Seq[(String, Array[Byte])]): Unit = {
      ...
      (0 until expected.length).foreach { idx =>
        val key = headers(idx).getAs[String]("headerKey")
        val value = headers(idx).getAs[Array[Byte]]("headerValue")
        ...
        Random.nextInt(5) match {
          case 0 => // Add a new topic
            ...
            AddKafkaData(topics.toSet, d: _*)(
              message = s"Add topic $newStressTopic",
          ...
          case 1 if running =>
            ...
            AddKafkaData(topics.toSet, d: _*)(
              message = s"Delete topic $deletedTopic",
              topicAction = (topic, partition) => {
          ...
          case 2 => // Add new partitions
            AddKafkaData(topics.toSet, d: _*)(message = "Add partition", topicAction = (topic, partition) => {
