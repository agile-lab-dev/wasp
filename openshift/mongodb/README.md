...
```console
$ docker run --name some-mongo -d wasp/mongo:tag
```
...
```console
$ docker run -it --network some-network --rm mongo wasp/mongo --host some-mongo test
```
...
Example `stack.yml` for `wasp/mongo`:
...
```console
$ docker run --name some-mongo -d wasp/mongo --serviceExecutor adaptive
```
...
```console
$ docker run -it --rm wasp/mongo --help
```
...
```console
$ docker run --name some-mongo -d wasp/mongo --wiredTigerCacheSizeGB 1.5
```
...
```console
$ docker run --name some-mongo -v /my/custom:/etc/mongo -d wasp/mongo --config /etc/mongo/mongod.conf
```
...
$ docker run -it --rm --network some-network wasp/mongo \
...
```console
$ docker run --name some-mongo -e MONGO_INITDB_ROOT_PASSWORD_FILE=/run/secrets/mongo-root -d wasp/mongo
```
...
2.	Start your `mongo` container like this:
	...
	```console
	$ docker run --name some-mongo -v /my/own/datadir:/data/db -d wasp/mongo
	```
