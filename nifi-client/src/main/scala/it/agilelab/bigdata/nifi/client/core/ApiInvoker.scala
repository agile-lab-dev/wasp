...
) extends SttpJson4sApi

class HttpException(val statusCode: StatusCode, val statusText: String, val message: String)
    extends Exception(s"[$statusCode] $statusText: $message")
...
object ApiInvoker {
  ...
  /** Allows request execution without calling apiInvoker.execute(request) request.result can be used to get a monad
    * wrapped content.
    *
    * @param request
    *   the apiRequest to be executed
    */
  implicit class ApiRequestImprovements[R[_], T](
      request: RequestT[Identity, Either[ResponseError[Exception], T], Nothing]
  ) {
