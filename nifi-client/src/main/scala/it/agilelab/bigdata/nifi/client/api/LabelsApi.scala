...
class LabelsApi(baseUrl: String)(implicit serializer: SttpSerializer) {
   ...
   */
  def removeLabel(id: String, version: Option[String] = None, clientId: Option[String] = None, disconnectedNodeAcknowledged: Option[Boolean] = None): ApiRequestT[LabelEntity] =
    basicRequest
      .method(Method.DELETE, uri"$baseUrl/labels/${id}?version=$version&clientId=$clientId&disconnectedNodeAcknowledged=$disconnectedNodeAcknowledged")
      .contentType("application/json")
      .response(asJson[LabelEntity])
