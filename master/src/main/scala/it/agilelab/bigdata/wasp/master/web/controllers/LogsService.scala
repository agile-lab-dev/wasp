...
trait LogsService {
  def logs(search: String, startTimestamp: Instant, endTimestamp: Instant, page: Int, size: Int): Future[Logs]

}

class DefaultSolrLogsService(client: SolrClient)(implicit ec: ExecutionContext) extends LogsService {
  ...
  override def logs(
    ...
    client.runPredicate(SolrLoggerIndex().name, query, size, page).map { response =>
