...
trait EventsService {
  def events(search: String, startTimestamp: Instant, endTimestamp: Instant, page: Int, size: Int): Future[Events]

}

class DefaultSolrEventsService(client: SolrClient)(implicit
    ec: ExecutionContext
  ...
  override def events(
    ...
    client.runPredicate(SolrEventIndex().name, query, size, page).map { response =>
