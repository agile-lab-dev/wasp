...
/** It read data from JDBC with the configuration of JDBCConfiguration. It use the push down method of SparkSQL to
  * convert SQL to JDBC query
  *
  * @param sqlModel
  *   JDBC configuration
  */
class JDBCSparkBatchReader(sqlModel: SqlSourceModel) extends SparkBatchReader with JdbcConfiguration with Logging {
  ...
  override def read(sc: SparkContext): DataFrame = {
    ...
    logger.info(
      s"Initialize Spark JDBCReader with " +
        s"\n\tconfig (obfuscated): $connectionObfuscated" +
        s"\n\tmodel: $sqlModel"
    )

    val readerOptions: Map[String, String] = getReaderOptions
    val readerOptionsObfuscated =
      readerOptions.map(ro => if ((ro._1 == "user") || (ro._1 == "password")) ro._1 -> "***" else ro)
  ...
  private def getReaderOptions = {
    ...
    val optPartitioningInfo = sqlModel.partitioningInfo
      .map(pi =>
        Map(
    ...
    val optNumPartitions = sqlModel.numPartitions.map(np => "numPartitions" -> np.toString).toMap

    val optFetchSize = sqlModel.fetchSize.map(fs => "fetchsize" -> fs.toString).toMap
