...
/** * On top level, the converters provide three high level interface.
  ...
  *   2. createConverterToSQL: Returns a function that is used to convert avro types to their corresponding sparkSQL
  *      representations.
...
object SchemaConverters {
      ...
      case UNION =>
        if (avroSchema.getTypes.asScala.exists(_.getType == NULL)) {
          ...
          if (remainingUnionTypes.size == 1) {
            toSqlType(remainingUnionTypes(0)).copy(nullable = true)
          } else {
          ...
          avroSchema.getTypes.asScala.map(_.getType) match {
            ...
            case other => throw new SchemaConversionException(s"This mix of union types is not supported: $other")
          }
      ...
      case MAP =>
        ...
        (item: Any) =>
          if (item == null) {
            ...
            item.asInstanceOf[HashMap[Any, Any]].asScala.map(x => (x._1.toString, valueConverter(x._2))).toMap
          }
      case UNION =>
        if (schema.getTypes.asScala.exists(_.getType == NULL)) {
          val remainingUnionTypes = schema.getTypes.asScala.filterNot(_.getType == NULL)
          if (remainingUnionTypes.size == 1) {
            createConverterToSQL(remainingUnionTypes(0))
          } else {
          ...
          schema.getTypes.asScala.map(_.getType) match {
            ...
            case other =>
              throw new SchemaConversionException(s"This mix of union types is not supported (see README): $other")
          }
  ...
  /** This function constructs converter function for a given sparkSQL datatype. This is used in writing Avro records
    * out to disk
    */
  def createConverterToAvro(dataType: DataType, structName: String, recordNamespace: String): (Any) => Any = {
    dataType match {
      ...
      case structType: StructType =>
        ...
        val fieldConverters =
          structType.fields.map(field => createConverterToAvro(field.dataType, field.name, recordNamespace))
        (item: Any) => {
          if (item == null) {
            null
          } else {
