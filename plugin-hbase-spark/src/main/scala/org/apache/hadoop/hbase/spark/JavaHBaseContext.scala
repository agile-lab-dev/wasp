...
/** This is the Java Wrapper over HBaseContext which is written in Scala. This class will be used by developers that
  * want to work with Spark or Spark Streaming in Java
  *
  * @param jsc
...
class JavaHBaseContext(@(transient @param) jsc: JavaSparkContext, @(transient @param) config: Configuration)
    extends Serializable {
  val hbaseContext = new HBaseContext(jsc.sc, config)

  /** A simple enrichment of the traditional Spark javaRdd foreachPartition. This function differs from the original in
    * that it offers the developer access to a already connected Connection object
    *
    ...
    */
  def foreachPartition[T](javaRdd: JavaRDD[T], f: VoidFunction[(java.util.Iterator[T], Connection)]) = {
  ...
  /** A simple enrichment of the traditional Spark JavaRDD mapPartition. This function differs from the original in that
  ...
  def mapPartitions[T, R](
      javaRdd: JavaRDD[T],
      f: FlatMapFunction[(java.util.Iterator[T], Connection), R]
  ): JavaRDD[R] = {

    def fn = (it: Iterator[T], conn: Connection) => f.call((it.asJava, conn)).asScala

    JavaRDD.fromRDD(
      hbaseContext.mapPartitions(
        javaRdd.rdd,
        (iterator: Iterator[T], connection: Connection) => fn(iterator, connection)
      )(fakeClassTag[R])
    )(fakeClassTag[R])
  }
  ...
  /** A simple abstraction over the HBaseContext.foreachPartition method.
    *
    * It allow addition support for a user to take JavaRDD and generate puts and send them to HBase. The complexity of
  ...
  /** A simple abstraction over the HBaseContext.foreachPartition method.
    ...
    */
  def bulkDelete[T](javaRdd: JavaRDD[T], tableName: TableName, f: Function[T, Delete], batchSize: Integer) {
    hbaseContext.bulkDelete(javaRdd.rdd, tableName, (t: T) => f.call(t), batchSize)
  }

  /** A simple abstraction over the HBaseContext.mapPartition method.
    *
    * It allow addition support for a user to take a JavaRDD and generates a new RDD based on Gets and the results they
  ...
  def hbaseRDD[U](tableName: TableName, scans: Scan, f: Function[(ImmutableBytesWritable, Result), U]): JavaRDD[U] = {
    JavaRDD.fromRDD(
      hbaseContext
        .hbaseRDD[U](tableName, scans, (v: (ImmutableBytesWritable, Result)) => f.call(v._1 -> v._2))(fakeClassTag[U])
    )(fakeClassTag[U])
  }
  ...
  /** Produces a ClassTag[T], which is actually just a casted ClassTag[AnyRef].
    ...
    * For instance, an Array[AnyRef] can hold any type T, but may lose primitive specialization.
    */
