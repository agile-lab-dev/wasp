...
/** HBaseRDDFunctions contains a set of implicit functions that can be applied to a Spark RDD so that we can easily
  * interact with HBase
  */
...
object HBaseRDDFunctions {
  ...
  implicit class GenericHBaseRDDFunctions[T](val rdd: RDD[T]) {
    ...
    /** Implicit method that gives easy access to HBaseContext's bulk put. This will not return a new RDD. Think of it
      * like a foreach
      *
      * @param hc
    ...
    /** Implicit method that gives easy access to HBaseContext's bulk get. This will return a new RDD. Think about it as
      * a RDD map function. In that every RDD value will get a new value out of HBase. That new value will populate the
      * newly generated RDD.
      *
      * @param hc
      *   The hbaseContext object to identify which HBase cluster connection to use
    ...
    /** Implicit method that gives easy access to HBaseContext's foreachPartition method. This will ack very much like a
      * normal RDD foreach method but for the fact that you will now have a HBase connection while iterating through the
      * values.
      *
      * @param hc
      *   The hbaseContext object to identify which HBase cluster connection to use
    ...
    /** Implicit method that gives easy access to HBaseContext's mapPartitions method. This will ask very much like a
      * normal RDD map partitions method but for the fact that you will now have a HBase connection while iterating
      * through the values
      *
      * @param hc
      *   The hbaseContext object to identify which HBase cluster connection to use
      ...
      */
    def hbaseMapPartitions[R: ClassTag](hc: HBaseContext, f: (Iterator[T], Connection) => Iterator[R]): RDD[R] = {
