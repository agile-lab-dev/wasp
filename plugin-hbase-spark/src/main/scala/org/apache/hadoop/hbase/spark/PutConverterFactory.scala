...
case class PutConverterFactory(@transient catalog: HBaseTableCatalog, schema: StructType) {
  ...
  val colsIdxedFields: Seq[(Int, Field)] = schema.fieldNames
  ...
  def convertToPut(row: InternalRow): Put = {
    ...
    val clusteringQualifierPrefixMap = clusteringCfColumnsMap.map { case (cf: String, clustFields: Seq[String]) =>
      val cqPrefix = clustFields
        .map { fieldName =>
...
object PutConverterFactory {
  ...
  private def getFields(catalog: HBaseTableCatalog, schema: StructType): (Seq[Field], Seq[Field]) = {
    ...
    val colsFields: Seq[Field] = schema.fieldNames
    ...
    val toAvroColumns = toAvroFields.map { f =>
      val avroConverterExpressionGenerator: (Expression, StructType) => AvroSerializerExpression =
        if (catalog.get("useAvroSchemaManager").exists(_.toBoolean)) {
          ...
          AvroSerializerExpression(ConfigManager.getAvroSchemaManagerConfig, avroSchema, f.colName, "wasp")
        } else {
      ...
      new Column(
        avroConverterExpressionGenerator(
          data.col(f.colName).expr,
          data.schema(f.colName).dataType.asInstanceOf[StructType]
        )
      ).as(f.colName)
    }
