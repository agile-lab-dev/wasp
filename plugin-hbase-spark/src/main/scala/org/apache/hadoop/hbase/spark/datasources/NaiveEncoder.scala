...
/** This is the naive non-order preserving encoder/decoder. Due to the inconsistency of the order between java primitive
  * types and their bytearray. The data type has to be passed in so that the filter can work correctly, which is done by
  * wrapping the type into the first byte of the serialized array.
  */
...
class NaiveEncoder extends BytesEncoder with Logging {
  ...
  /** Evaluate the java primitive type and return the BoundRanges. For one value, it may have multiple output ranges
    * because of the inconsistency of order between java primitive type and its byte array order.
    *
    * For short, integer, and long, the order of number is consistent with byte array order if two number has the same
    * sign bit. But the negative number is larger than positive number in byte array.
    *
    * For double and float, the order of positive number is consistent with its byte array order. But the order of
    * negative number is the reverse order of byte array. Please refer to IEEE-754 and
  ...
  def ranges(in: Any): Option[BoundRanges] = in match {
    ...
    case a: Double =>
      ...
      if (a >= 0.0f) {
        Some(
          BoundRanges(
            Array(BoundRange(Bytes.toBytes(0.0d), b), BoundRange(Bytes.toBytes(-0.0d), Bytes.toBytes(Double.MinValue))),
        ...
        Some(
          BoundRanges(
            ...
            Array(BoundRange(Bytes.toBytes(-0.0d), b), BoundRange(Bytes.toBytes(0.0d), Bytes.toBytes(Double.MaxValue))),
            b
          )
        )
      }
    case a: Float =>
      ...
      if (a >= 0.0f) {
        Some(
          BoundRanges(
            Array(BoundRange(Bytes.toBytes(0.0f), b), BoundRange(Bytes.toBytes(-0.0f), Bytes.toBytes(Float.MinValue))),
        ...
        Some(
          BoundRanges(
            ...
            Array(BoundRange(Bytes.toBytes(-0.0f), b), BoundRange(Bytes.toBytes(0.0f), Bytes.toBytes(Float.MaxValue))),
            b
          )
        )
      }
  ...
  /** encode the data type into byte array. Note that it is a naive implementation with the data type byte appending to
    * the head of the serialized byte array.
    *
    * @param dt:
  ...
  override def filter(
    ...
    filterBytes(offset2) match {
      ...
      case _ =>
        ...
        compare(Bytes.compareTo(input, offset1, length1, filterBytes, offset2 + 1, length2 - 1), ops)
    }
  }
}
