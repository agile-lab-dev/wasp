...
case class BacklogAnalyzerConfig(pipegraph: PipegraphModel, etls: List[StructuredStreamingETLModel])
  ...
  def fromConfig(conf: Config, pipegraphs: Map[String, PipegraphModel]): Either[String, BacklogAnalyzerConfig] = {
    ...
    def eModels(pipegraphModel: PipegraphModel): Either[String, List[StructuredStreamingETLModel]] = {
      ...
      kafkaInput match {
        case Nil =>
          Left(
            s"Pipegraph ${pipegraphModel.name} did not contain any etl which reads from Kafka, " +
