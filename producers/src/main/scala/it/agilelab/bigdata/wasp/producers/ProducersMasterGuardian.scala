...
class ProducersMasterGuardian(env: { val producerBL: ProducerBL; val topicBL: TopicBL })
  ...
  private def retrieveProducers: Map[String, ActorRef] =
    retrieveSystemAndLocalProducers ++ remoteProducersComponentActors
  ...
  private def retrieveSystemAndLocalProducers: Map[String, ActorRef] = {
    env.producerBL.getAll    // grab all producers
      ...
      .map(producer => {
        ...
        if (producer.name == InternalLogProducerGuardian.name) { // logger producer is special
          ...
          if (localProducerComponentActors.contains(producerName))
            producerName -> localProducerComponentActors(
              producerName
            ) // do not instantiate, but get the already existing one from localProducerComponentActors
          else {
  ...
  override def preStart(): Unit = {
    ...
    if (waspConfig.systemProducersStart) {
      ...
      env.producerBL.getSystemProducers foreach { producer =>
        {
          logger.info(s"Scheduling startup of system producer '${producer.name}'")
          WaspSystem.masterGuardian ! StartProducer(
            producer.name
          ) // masterGuardian uses producer.name instead of producer._id.get.getValue.toHexString
        }
      }
  ...
  private def restProducerRequest(request: RestProducerRequest, producer: ProducerModel): Either[String, String] = {
    ...
    if (producers.isDefinedAt(producer.name)) {
      ...
      ??[Either[String, Unit]](
        ...
        RestRequest(request.httpMethod, request.data, request.modelKey)
      ) match {
        case Right(_) =>
