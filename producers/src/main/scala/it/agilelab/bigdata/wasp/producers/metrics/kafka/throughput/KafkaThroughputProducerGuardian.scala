...
abstract class KafkaThroughputProducerGuardian[A](
  ...
  def this(env: { val producerBL: ProducerBL; val topicBL: TopicBL }, producerName: String) = this(
    env,
    producerName,
  ...
  ): KafkaThroughputProducerActor[A]

  private def spawnKafkaOffsetCheckerActor(
      kafkaActor: ActorRef,
      topicToCheck: String,
      triggerInterval: Long,
      windowSize: Long,
    ...
    logger.info(s"""Created actor KafkaThroughputProducerActor. It will follow this configuration:
  ...
  /** Method to pick the topics you want to monitor. Default implementation looks for the configuration:
    * wasp.kafkaThroughput.topics which is expected to be a list of conf which should contain the following fields <ul>
    * <li>topicName: The topic name to monitor</li> <li>triggerIntervalMs: how many millis should pass between an offset
    * check and the next (i.e. how frequently kafka is going to be contacted)</li> <li>windowSizeMs: window over the
    * throughput will be calculated</li> <li>sendMessageEvery: every how many offset check a new message should be
    * emitted</li> </ul>
    *
    * @return
    *   Right list of topics to monitor, Left error message
    */
