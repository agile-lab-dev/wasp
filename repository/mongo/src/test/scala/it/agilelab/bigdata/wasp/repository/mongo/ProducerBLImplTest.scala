...
class ProducerBLImplTest extends FlatSpec with Matchers{
  ...
  it should "test ProducerBLImpl for Mongo" in {

    val db = WaspMongoDB
    db.initializeDB()
    val waspDB = db.getDB()
    val bl = new ProducerBLImp(waspDB)
...
//    an [SQLException] should be thrownBy bl.persist(p3Bis)  ------ different ObjectID so no conflict
