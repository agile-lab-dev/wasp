...
import it.agilelab.bigdata.wasp.repository.core.mappers.{
  KeyValueMapperSelector,
  KeyValueMapperV1,
  RawMapperSelector,
  RawMapperV1
}
...
object DataStoreConfCodecProviders {
    ...
    override def encodeWithType(
      ...
      value match {
        case kvd: KeyValueDataStoreConf =>
          createBsonDocument(
            registry.get(classOf[KeyValueDataStoreConf]),
            KeyValueDataStoreConf.TYPE,
            kvd,
            encoderContext
          )
  ...
  object RawDataStoreConfCodecProvider extends AbstractCodecProvider[RawDataStoreConf] {
    override def decodeClass(
      ...
      RawDataStoreConf(
        ...
        readObject(
          nameOf[RawDataStoreConf](_.partitionPruningStrategy),
          registry.get(classOf[PartitionPruningStrategy])
        )
      )
    }

    override def encodeClass(registry: CodecRegistry, rawDataStoreConf: RawDataStoreConf)(implicit
        writer: BsonWriter,
        encoderContext: EncoderContext
    ): Unit = {
      ...
      writeObject(
        nameOf[RawDataStoreConf](_.rawModel),
        RawMapperV1.transform[RawDBModelV1](rawDataStoreConf.rawModel),
        registry.get(classOf[RawDBModel])
      )
      writeObject(
        nameOf[RawDataStoreConf](_.rawMatchingStrategy),
        rawDataStoreConf.rawMatchingStrategy,
        registry.get(classOf[RawMatchingStrategy])
      )
      writeObject(
        nameOf[RawDataStoreConf](_.partitionPruningStrategy),
        rawDataStoreConf.partitionPruningStrategy,
        registry.get(classOf[PartitionPruningStrategy])
      )
    }
  ...
  object KeyValueDataStoreConfCodecProvider extends AbstractCodecProvider[KeyValueDataStoreConf] {
    override def decodeClass(
      ...
      val keyValueModel =
        readObject(nameOf[KeyValueDataStoreConf](_.keyValueModel), registry.get(classOf[KeyValueDBModel]))
      ...
      KeyValueDataStoreConf(
        ...
        readObject(
          nameOf[KeyValueDataStoreConf](_.keyValueMatchingStrategy),
          registry.get(classOf[KeyValueMatchingStrategy])
        )
      )
    }

    override def encodeClass(registry: CodecRegistry, keyValueDataStoreConf: KeyValueDataStoreConf)(implicit
        writer: BsonWriter,
        encoderContext: EncoderContext
    ): Unit = {
      ...
      writer.writeString(
        nameOf[KeyValueDataStoreConf](_.correlationIdColumn),
        keyValueDataStoreConf.correlationIdColumn
      )

      writeObject(
        nameOf[KeyValueDataStoreConf](_.keyValueModel),
        KeyValueMapperV1.transform[KeyValueDBModelV1](keyValueDataStoreConf.keyValueModel),
        registry.get(classOf[KeyValueDBModel])
      )
      writeObject(
        nameOf[KeyValueDataStoreConf](_.keyValueMatchingStrategy),
    ...
    override def encodeWithType(
      ...
      value match {
        case exact: ExactKeyValueMatchingStrategy =>
          createBsonDocument(
            registry.get(classOf[ExactKeyValueMatchingStrategy]),
            ExactKeyValueMatchingStrategy.TYPE,
            exact,
            encoderContext
          )
        case prefix: PrefixKeyValueMatchingStrategy =>
          createBsonDocument(
            registry.get(classOf[PrefixKeyValueMatchingStrategy]),
            PrefixKeyValueMatchingStrategy.TYPE,
            prefix,
            encoderContext
          )
        case prefixAndTime: PrefixAndTimeBoundKeyValueMatchingStrategy =>
          createBsonDocument(
            registry.get(classOf[PrefixAndTimeBoundKeyValueMatchingStrategy]),
            PrefixAndTimeBoundKeyValueMatchingStrategy.TYPE,
            prefixAndTime,
            encoderContext
          )
      }
    ...
    override def encodeWithType(
      ...
      value match {
        case exact: ExactRawMatchingStrategy =>
          createBsonDocument(
            registry.get(classOf[ExactRawMatchingStrategy]),
            ExactRawMatchingStrategy.TYPE,
            exact,
            encoderContext
          )
        case prefix: PrefixRawMatchingStrategy =>
          createBsonDocument(
            registry.get(classOf[PrefixRawMatchingStrategy]),
            PrefixRawMatchingStrategy.TYPE,
            prefix,
            encoderContext
          )
        case contains: ContainsRawMatchingStrategy =>
          createBsonDocument(
            registry.get(classOf[ContainsRawMatchingStrategy]),
            ContainsRawMatchingStrategy.TYPE,
            contains,
            encoderContext
          )
      }
    ...
    override def encodeWithType(
      ...
      value match {
        case exact: TimeBasedBetweenPartitionPruningStrategy =>
          createBsonDocument(
            registry.get(classOf[TimeBasedBetweenPartitionPruningStrategy]),
            TimeBasedBetweenPartitionPruningStrategy.TYPE,
            exact,
            encoderContext
          )
        case prefix: NoPartitionPruningStrategy =>
          createBsonDocument(
            registry.get(classOf[NoPartitionPruningStrategy]),
            NoPartitionPruningStrategy.TYPE,
            prefix,
            encoderContext
          )
      }
