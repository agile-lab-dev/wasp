...
import it.agile.bigdata.wasp.repository.postgres.tables.{PipegraphInstanceTableDefinition, TableDefinition}
...
case class PipegraphInstanceBlImpl(waspDB : WaspPostgresDB) extends PipegraphInstanceBl with PostgresBL {
  ...
  override def instancesOf(name: String): Seq[PipegraphInstanceModel] = waspDB.getBy(s"${PipegraphInstanceTableDefinition.instanceOf}='$name'")
