...
case class MlModelBLImpl(waspDB: WaspPostgresDB) extends MlModelBL with PostgresBL {
  ...
  implicit override val tableDefinition: TableDefinition[MlModelOnlyInfo, (String, String, Long)] =
    MlModelOnlyInfoTableDefinition
  val tableData: TableDefinition[(BsonObjectId, String, BsonDocument, Array[Byte]), BsonObjectId] =
    MlModelOnlyDataTableDefinition
  ...
  override def getMlModelOnlyInfo(name: String, version: String): Option[MlModelOnlyInfo] = {
    waspDB
      .getBy(
        Array((MlModelOnlyInfoTableDefinition.name, name), (MlModelOnlyInfoTableDefinition.version, version)),
        Some(s"${MlModelOnlyInfoTableDefinition.timestamp} DESC"),
        Some(1)
      )(tableDefinition)
      .headOption
  }
