...
trait PostgresDBHelper extends ConnectionSupport with ConnectionInfoProvider with Logging {
  ...
  private def resultSetInterator[S](resultSet: ResultSet, mapper: (ResultSet) => S): Seq[S] = {
    Iterator
      .continually { resultSet != null && resultSet.next() }
      .takeWhile(hasNext => hasNext)
      .map(_ => { mapper(resultSet) })
      .toStream
      .toList
  }

  private[postgres] def selectAll[T](
      table: String,
      columns: Array[String],
      whereCondition: Option[Array[(String, Any)]] = None,
      sortCondition: Option[String] = None,
      limitOption: Option[Int] = None
  )(mapper: ResultSet => T): Seq[T] = {
  ...
  protected def insert[T](table: String, obj: T)(mapper: T => Array[(String, Any)]): Unit = {
    ...
    execute(
      s"INSERT INTO $table (${objTransformed.map(_._1).mkString(",")}) VALUES (${objTransformed.map(e => "?").mkString(",")})",
      objTransformed.map(_._2)
    )
  }

  protected def insertReturning[T, R](table: String, obj: T, columnsResult: Array[String])(
      mapperFromModel: T => Array[(String, Any)],
      mapperFromResultSet: ResultSet => R
  ): Seq[R] = {
    val objTransformed = mapperFromModel(obj)
    val query =
      s"INSERT INTO $table(${objTransformed.map(_._1).mkString(",")}) VALUES(${objTransformed.map(_ => "?").mkString(",")}) RETURNING ${columnsResult.mkString(",")}"
    executeQuery(query, objTransformed.map(_._2))(mapperFromResultSet)
  }

  protected def updateBy[T](table: String, obj: T, whereCondition: Array[(String, Any)])(
      mapper: T => Array[(String, Any)]
  ): Unit = {
    val objTransformed = mapper(obj)
    val where          = s" WHERE ${whereCondition.map(s => s"${s._1}=?").mkString(" AND ")} "
    execute(
      s"update $table SET ${objTransformed.map(e => s"${e._1} = ?").mkString(",")} $where",
