...
case class ConfigManagerBLImpl(waspDB: WaspPostgresDB) extends ConfigManagerBL with PostgresBL {
  ...
  implicit override val tableDefinition: TableDefinition[Model, String] = ConfigManagerTableDefinition

  override def getByName[T <: Model](
      name: String
  )(implicit ct: ClassTag[T], typeTag: universe.TypeTag[T]): Option[T] = {
    waspDB.getByPrimaryKey(name).map(_.asInstanceOf[T])
  }

  override def retrieveConf[T <: Model](default: T, nameConf: String)(implicit
      ct: ClassTag[T],
      typeTag: universe.TypeTag[T]
  ): Option[T] = {
  ...
  override def retrieveDBConfig(): Seq[String] =
    waspDB.selectAll(tableDefinition.tableName, Array(ConfigManagerTableDefinition.getPayload), None)(rs =>
      rs.getString(ConfigManagerTableDefinition.getPayload)
    )
}
