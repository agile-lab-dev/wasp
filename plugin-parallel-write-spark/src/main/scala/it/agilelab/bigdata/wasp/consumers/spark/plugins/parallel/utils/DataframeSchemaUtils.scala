...
object DataframeSchemaUtils {
  ...
  /** Manipulates a dataframe to be compliant with a schema, first selects the dataframe than checks that the selected
    * schema is compliant with the provided schema
    * @param df
    *   Dataframe to manipulate
    * @param schema
    *   schema that must be enforced
    * @return
    *   Either a converted dataframe or a throwable
    */
