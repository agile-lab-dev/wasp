...
object ParallelWriterFactory {
  ...
  private def getColdWriter(
    ...
    writerDetails match {
      case parallelWrite: ParallelWrite =>
        getParallelWriter(format, parallelWrite, entityAPI, entityDetails, catalogService)
