...
trait EntityCatalogBuilder {
  ...
  /** Gets microservice catalog class name from configurations, instantiate the class and cast to
    * MicroServiceCatalogService
    * @param configPath
    *   Path in configurations containing MicroserviceCatalogService class name
    * @tparam T
    *   Type of microservice
    * @return
    *   Instance of MicroserviceCatalogService
    */
  protected def getEntityCatalogService(configPath: String): EntityCatalogService = {
    ...
    getEntityCatalogServiceInstance(className).recover {
      case classNotFoundException: ClassNotFoundException =>
        throw new ClassNotFoundException(s"No class with name '$className\' found")
      case reflectiveOperationException: ReflectiveOperationException =>
        throw new InstantiationException(s"Could not create instance of $className")
      case classCastException: ClassCastException =>
        throw new ClassCastException(s"$className does not extend MicroserviceCatalogService")
  ...
  private def instantiateService(clazz: Class[_]): Try[EntityCatalogService] = Try(
    clazz.getDeclaredConstructor().newInstance().asInstanceOf[EntityCatalogService]
  )
...
object EntityCatalogBuilder extends EntityCatalogBuilder {
  ...
  /** Obtain platform catalog service class from configuration and instantiate it by reflection
    * @tparam T
    *   Type of microservice getted by catalog service
    * @return
    *   Instance of microservice catalog service
    */
  def getEntityCatalogService(): EntityCatalogService = {
